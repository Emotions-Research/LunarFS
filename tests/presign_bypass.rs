...
use devdropbox::presign::{
    fetch_presigned, put_presigned, resolve_stub, LocalStubPresigner, PresignError, PresignOp,
    Presigner,
};
...
impl Env {
    fn new() -> Self {
        ...
        let store =
            Arc::new(LocalFileSystem::new_with_prefix(dir.path()).expect("LocalFileSystem"));
...
async fn bypass_put_then_get_roundtrip() {
    ...
    assert_eq!(
        on_disk, data,
        "on-disk bytes must be byte-identical to uploaded data"
    );
    ...
    let fetched = fetch_presigned(&client, &get_url, NOW)
        .await
        .expect("fetch_presigned must succeed");
    assert_eq!(
        fetched, data,
        "fetched bytes must be byte-identical to uploaded data"
    );
...
async fn expired_presign_rejected() {
    ...
    let signed = p
        .presign(PresignOp::Put, object_path, 10, 1000)
        .expect("presign");
    ...
    assert!(
        fetch_err.is_err(),
        "fetch_presigned with expired URL must return Err"
    );
    let msg = format!("{}", fetch_err.unwrap_err());
    assert!(
        msg.contains("expired"),
        "error message must mention expiry, got: {}",
        msg
    );
}
...
async fn invalid_presign_rejected() {
    ...
    assert!(
        fetch_err.is_err(),
        "fetch_presigned with tampered URL must return Err"
    );
    let msg = format!("{}", fetch_err.unwrap_err());
    assert!(
...
async fn full_client_roundtrip_via_tcp() {
    ...
    let tok = {
        ...
        let ws_id =
            repo::create_workspace(&c, "ws", OwnerKind::User, uid, NOW).expect("create_workspace");
        acl::grant(
            &c,
            PrincipalKind::User,
            &uid.to_string(),
    ...
    assert!(
        ready,
        "in-process server must become ready within 2s on {}",
        addr
    );
    ...
    remote
        .put_blob(&hash, data.to_vec(), Some("ws"))
        .await
        .expect("put_blob via presign");
    ...
    let fetched = remote
        .get_blob(&hash, Some("ws"))
        .await
        .expect("get_blob via presign");
    assert_eq!(
        fetched, data,
        "fetched bytes must be byte-identical to uploaded data"
    );
