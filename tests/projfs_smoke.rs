...
fn lunar_bin() -> String {
    std::env::var("CARGO_BIN_EXE_lunar").unwrap_or_else(|_| "lunar".to_string())
}
...
fn make_fixture(salt: u8, len: usize) -> Vec<u8> {
    assert!(
        len <= 16 * 1024 * 1024,
        "fixture len must not exceed 16 MiB"
    );
...
fn projfs_mount_probe() -> bool {
    ...
    assert!(
        !sentinel_bytes.is_empty(),
        "probe sentinel must not be empty"
    );
...
fn wait_for_ready(sentinel_path: &std::path::Path, expected_bytes: &[u8]) {
    assert!(
        ...
        "sentinel content must not be empty"
    );
...
mod detect {
    ...
    pub fn probe_projfs_available() -> std::io::Result<ProjfsAvailability> {
        let probe_dir =
            tempfile::tempdir().map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
...
fn projfs_lazy_hydration_byte_correctness() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !meta.is_dir(),
        "projfs_smoke: fixture.bin must not be reported as a directory"
    );
    eprintln!(
        "projfs_smoke: placeholder metadata correct (size={})",
        meta.len()
    );
...
fn projfs_concurrent_read_stress() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::fs::write(repo_dir.path().join("sentinel.txt"), sentinel_bytes).expect("write sentinel");
    ...
    assert_eq!(
        fixtures.len(),
        K,
        "must have exactly K fixture files before mounting"
    );
