...
fn smoke_s3_remote() {
    ...
    let remote =
        Remote::from_store(Arc::new(store), "smoke-ws").expect("Remote::from_store must succeed");
    ...
    let got_head = remote
        .read_head()
        .unwrap()
        .expect("HEAD must exist after write");
    assert_eq!(
        got_head.root, root,
        "read_head must return the written root"
    );
