...
fn run_fuse_mount_smoke() {
    ...
    std::fs::write(
        repo_dir.path().join("smoke-sentinel.txt"),
        b"lunar-smoke-ok",
    )
    .expect("write sentinel");
    std::fs::create_dir_all(repo_dir.path().join("sub")).expect("create subdir");
    std::fs::write(
        repo_dir.path().join("sub").join("data.txt"),
    ...
    assert!(
        ready,
        "smoke_real_mount: FUSE mount never became ready within 15 s"
    );
    ...
    assert_eq!(
        ...
        "nested file content must match"
    );
...
fn run_mcp_exchange_smoke() {
    ...
    let init_val: serde_json::Value =
        serde_json::from_str(&init_response).expect("MCP initialize response must be valid JSON");
    assert_eq!(
        init_val["id"],
        serde_json::json!(1),
    ...
    let server_name = init_val["result"]["serverInfo"]["name"]
        .as_str()
        .unwrap_or("");
    assert!(
    ...
    eprintln!(
        "smoke_real_mount: MCP initialize OK (server: {})",
        server_name
    );
    ...
    let list_val: serde_json::Value =
        serde_json::from_str(&list_response).expect("MCP tools/list response must be valid JSON");
    assert_eq!(
        list_val["id"],
        serde_json::json!(2),
    ...
    assert!(!tools.is_empty(), "lunar-mcp must expose at least one tool");
...
fn write_mcp_frame(writer: &mut impl Write, body: &str) {
    ...
    assert!(
        body.len() <= 1_048_576,
        "MCP frame body exceeds 1 MiB sanity cap"
    );
    let header = format!("Content-Length: {}\r\n\r\n", body.len());
    writer
        .write_all(header.as_bytes())
        .expect("MCP frame header write must succeed");
    writer
        .write_all(body.as_bytes())
        .expect("MCP frame body write must succeed");
...
fn read_mcp_frame(reader: &mut impl BufRead) -> String {
    ...
    for _ in 0..32usize {
        ...
        reader
            .read_line(&mut line)
            .expect("MCP response header read must succeed");
    ...
    for _ in 0..1024usize {
        ...
        assert!(
            read > 0 || offset >= n,
            "MCP response body read returned 0 unexpectedly"
        );
        offset += read;
    }
    assert_eq!(
        offset, n,
        "MCP response body must be exactly Content-Length bytes"
    );
