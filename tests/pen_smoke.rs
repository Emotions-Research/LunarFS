...
fn bootstrap_db(db_path: &std::path::Path) -> (String, String, String, i64) {
    ...
    let ws_a_id = repo::create_workspace(&conn, "ws-a", OwnerKind::User, uid_a, 0)
        .expect("create ws-a");
    acl::grant(
        &conn,
        PrincipalKind::User,
    ...
    let tok_a = token::mint(
        &conn,
        &Principal { kind: OwnerKind::User, id: uid_a.to_string() },
        None,
        None,
        &clock,
    )
    ...
    let ws_b_id = repo::create_workspace(&conn, "ws-b", OwnerKind::User, uid_b, 0)
        .expect("create ws-b");
    acl::grant(
        &conn,
        PrincipalKind::User,
    ...
    let tok_b = token::mint(
        &conn,
        &Principal { kind: OwnerKind::User, id: uid_b.to_string() },
        None,
        None,
        &clock,
    )
    ...
    let tok_ro = token::mint(
        &conn,
        &Principal { kind: OwnerKind::User, id: uid_ro.to_string() },
        None,
        None,
        &clock,
    )
