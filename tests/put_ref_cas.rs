...
fn seed_workspace(state: &AppState, ws_name: &str) -> String {
    ...
    let ws_id =
        repo::create_workspace(&conn, ws_name, OwnerKind::User, uid, NOW)
...
async fn matching_expected_root_overwrites_with_200() {
    ...
    .await;
    assert_eq!(r.status(), StatusCode::OK, "CAS with matching expected_root must return 200");

    let stored = get_ref_value(state, &tok, "ws-match").await;
    assert_eq!(stored.as_deref(), Some(root_b.as_str()), "root_b must be stored after CAS success");
}
...
async fn mismatching_expected_root_creates_conflict_ref_and_returns_409() {
    ...
    .await;
    assert_eq!(r.status(), StatusCode::CONFLICT, "mismatching expected_root must return 409");
    ...
    assert_eq!(current_root, root_a, "current_root in 409 body must be root_a");

    // The workspace ref must NOT have been overwritten: still root_a.
    let stored = get_ref_value(state, &tok, "ws-conflict").await;
    assert_eq!(stored.as_deref(), Some(root_a.as_str()), "workspace ref must remain root_a after conflict");
