...
fn remote_on_access_hydration_exact_counts_and_sparse_store() {
    ...
    let root_bytes = serialize_tree(&[
        ...
        TreeEntry { mode: MODE_FILE, name: "shared_a.rs".into(), hash: h_shared },
        TreeEntry { mode: MODE_FILE, name: "shared_b.rs".into(), hash: h_shared },
        TreeEntry { mode: MODE_FILE, name: "unread.txt".into(), hash: h_unread },
    ]);
    ...
    assert_eq!(index.len(), 6, "6 files: README.md, src/main.rs, src/lib.rs, shared_a.rs, shared_b.rs, unread.txt");
    ...
    assert!(!store_b.has(&h_readme), "README blob must not be in store B before any read");
    assert!(!store_b.has(&h_main), "main.rs blob must not be in store B before any read");
    assert!(!store_b.has(&h_lib), "lib.rs blob must not be in store B before any read");
    assert!(!store_b.has(&h_shared), "shared blob must not be in store B before any read");
    assert!(!store_b.has(&h_unread), "unread blob must not be in store B before any read");
    ...
    assert_eq!(got_readme, content_readme, "README.md bytes must be byte-identical to source");
    assert_eq!(fetch_count(&counts, &h_readme), 1, "first read must trigger exactly one fetch");
    assert!(store_b.has(&h_readme), "README blob must be in store B after first read");
    ...
    assert_eq!(got_readme2, content_readme, "second read must return identical bytes");
    assert_eq!(fetch_count(&counts, &h_readme), 1, "second read must NOT trigger a new fetch");
    ...
    assert_eq!(fetch_count(&counts, &h_main), 1, "main.rs must have exactly one fetch");
    assert!(store_b.has(&h_main), "main.rs blob must be in store B after read");
    ...
    assert_eq!(fetch_count(&counts, &h_lib), 1, "lib.rs must have exactly one fetch");
    ...
    assert_eq!(got_shared_a, content_shared, "shared_a.rs bytes must match source");
    // First read of the shared blob triggers one fetch and caches it.
    assert_eq!(fetch_count(&counts, &h_shared), 1, "first access of shared blob must fetch once");
    ...
    assert_eq!(got_shared_b, content_shared, "shared_b.rs bytes must match source");
    ...
    assert_eq!(fetch_count(&counts, &h_unread), 0, "unread blob must never be fetched");
    assert!(
    ...
    // backed by the same server, proving that the FUSE overlay read path hydrates.
    {
        ...
        let index_fuse =
            Index::build(server.as_ref(), &root_hash).expect("index_fuse build");
        ...
        let result = route_read(&hydrating_fuse, &index_fuse, &overlay, agent, "README.md", 0, 1024)
            .expect("route_read must not error")
            .expect("README.md must be present via route_read");
        assert_eq!(result, content_readme, "FUSE route_read must return byte-identical content");
        assert!(
        ...
        let result2 =
            route_read(&hydrating_fuse, &index_fuse, &overlay, agent, "README.md", 0, 1024)
                .expect("second route_read must not error")
                .expect("path must still exist");
        assert_eq!(result2, content_readme, "second route_read must return same bytes");
...
fn hydrating_store_absent_remote_blob_is_not_found() {
    ...
    let result = hydrating.get(&phantom_hash).expect("absent remote must not Err");
    assert!(result.is_none(), "blob absent from both local and remote must return Ok(None)");
    assert!(!local.has(&phantom_hash), "absent blob must not appear in local CAS");
}
...
fn hydrating_store_local_hit_skips_remote() {
    ...
    let got = hydrating.get(&hash).expect("get must succeed").expect("blob must be present");
