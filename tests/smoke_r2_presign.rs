...
fn build_fixture() -> Fixture {
    ...
    let h_readme = store
        .put(b"# Dev Dropbox presign smoke\n")
        .expect("put README");
    let h_main = store
        .put(b"fn main() { println!(\"presign\"); }\n")
        .expect("put main.rs");
    let h_lib = store
        .put(b"pub fn greet() -> &'static str { \"presign\" }\n")
        .expect("put lib.rs");
...
async fn smoke_r2_presign_roundtrip() {
    ...
    assert!(
        ready,
        "lunar serve did not become ready within ~6s on {}",
        addr
    );
    ...
    let presigned = remote
        .presign(
            &fixture.root,
            devdropbox::presign::PresignOp::Get,
            Some(&workspace),
        )
        .await
    ...
    assert_eq!(
        ...
        "presign GET must return method GET"
    );
    ...
    eprintln!(
        "smoke_r2_presign: cleaned up {} blobs + ref object",
        deleted
    );
    ...
    assert_eq!(
        index_a.len(),
        index_b.len(),
        "file count must match after R2 presign roundtrip"
    );
    ...
    for (path, hash_a) in index_a.entries() {
        ...
        assert_eq!(
            *hash_a, hash_b,
            "hash mismatch for {} after R2 presign roundtrip",
            path
        );
