...
async fn contract_blobs_missing_route_registered() {
    ...
    assert_eq!(
        resp.status(),
        StatusCode::OK,
        "POST /v1/blobs/missing must return 200"
    );
...
async fn contract_ref_routes_registered_and_roundtrip() {
    ...
    assert_eq!(
        put_resp.status(),
        StatusCode::OK,
        "PUT /v1/ref/:workspace must return 200"
    );
    ...
    assert_eq!(
        get_resp.status(),
        StatusCode::OK,
        "GET /v1/ref/:workspace after PUT must return 200"
    );
    let v: serde_json::Value = serde_json::from_slice(&body_bytes(get_resp).await).expect("JSON");
    assert_eq!(
        v["root"].as_str().unwrap(),
        root_hex,
        "root hash must round-trip"
    );
}
...
async fn contract_blob_routes_registered_and_roundtrip() {
    ...
    assert_eq!(
        put_resp.status(),
        StatusCode::CREATED,
        "PUT /v1/blob/:hash must return 201"
    );
    ...
    assert_eq!(
        get_resp.status(),
        StatusCode::OK,
        "GET /v1/blob/:hash must return 200"
    );
    assert_eq!(
...
async fn contract_blob_presign_route_registered() {
    ...
    let v: serde_json::Value = serde_json::from_slice(&body_bytes(resp).await).expect("JSON");
    assert!(
        v["url"].as_str().is_some(),
        "presign response must include url"
    );
    assert_eq!(
        v["method"].as_str().unwrap(),
        "PUT",
        "op=put must yield method=PUT"
    );
}
