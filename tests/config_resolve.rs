...
fn make_temp_config() -> (TempDir, Config) {
    ...
    assert_eq!(
        ...
        Some("https://my.server.example.com")
    );
...
fn org_slash_workspace_resolves() {
    let (_dir, cfg) = make_temp_config();
    let env = HashMap::new();
    let target = resolve_workspace("myorg/ws", &cfg, &env).expect("org/workspace must resolve");
...
fn three_seg_host_scheme_prepended_when_absent() {
    ...
    assert_eq!(
        target.server, "https://self.example.com",
        "scheme must be prepended"
    );
    assert_eq!(target.org, "myorg");
    assert_eq!(target.workspace, "ws");
}
...
fn three_seg_host_existing_scheme_preserved() {
    ...
    assert_eq!(
        target.server, "http://self.example.com",
        "existing scheme must not be doubled"
    );
}
...
fn env_server_and_token_override_config() {
    ...
    env.insert(
        ...
        "https://override.server.com".to_string(),
    );
    env.insert(ENV_TOKEN.to_string(), "env_tok_xyz".to_string());
    let target =
        resolve_workspace("myorg/ws", &cfg, &env).expect("must resolve with env overrides");
    assert_eq!(
        target.server, "https://override.server.com",
        "env server must beat config"
    );
...
fn default_host_when_config_server_absent_and_no_env_override() {
    ...
    assert_eq!(
        target.server, DEFAULT_HOST,
        "must fall through to compiled DEFAULT_HOST"
    );
}
...
fn missing_token_yields_error_with_login_hint() {
    ...
    let env = HashMap::new();
    let err =
        resolve_workspace("ws", &loaded, &env).expect_err("missing token must yield an error");
...
fn missing_config_file_returns_empty_config() {
    ...
    let cfg = load_config_from_path(&path).expect("missing config file must not return an error");
...
fn env_org_override_used_for_bare_name() {
    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir.path().join("config");
    let cfg_no_org = Config {
        server: Some("https://s.example.com".to_string()),
        token: Some("tok".to_string()),
        org: None,
    };
