...
fn nfs_lifecycle_repeated_cycles_and_interrupt() {
    ...
    std::fs::write(
        repo_dir.path().join("lifecycle-sentinel.txt"),
        b"nfs-lifecycle-ok",
    )
...
fn run_cycle(
    ...
    let sig_name = if sig == libc::SIGINT {
        "SIGINT"
    } else {
        "SIGTERM"
    };
    ...
    let port = match port_rx.recv_timeout(Duration::from_secs(8)) {
        ...
        Err(_) => {
            ...
            let _ = child.wait();
            panic!(
                "smoke_nfs_lifecycle: cycle {}: port line not received within 8s",
                cycle
            );
        }
    };
    ...
    assert_eq!(
        rc, 0,
        "libc::kill(pid={}, sig={}) must succeed",
        child_pid, sig_name
    );
