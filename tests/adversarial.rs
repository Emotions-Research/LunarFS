...
fn make_workspace(
    ...
    let ws_id = repo::create_workspace(&conn, ws_name, OwnerKind::User, uid, T)
        .expect("create_workspace");
    acl::grant(&conn, PrincipalKind::User, &uid.to_string(), ws_id, "/", perm, T)
        .expect("acl grant");
    let tok = token::mint(
        &conn,
...
async fn authz_bypass_cross_workspace_read_denied() {
    ...
    let put = build_router(env.state.clone())
        ...
        .await
        .expect("oneshot");
    assert_eq!(put.status(), StatusCode::CREATED, "B writing own workspace must succeed");
    ...
    let get = build_router(env.state.clone())
        ...
        .await
        .expect("oneshot");
    assert_eq!(get.status(), StatusCode::FORBIDDEN, "A must not read B's workspace");
    let body = collect_body(get).await;
    // Confirm no blob bytes were leaked (body is error JSON, not the secret data).
    assert!(!body.as_ref().windows(data.len()).any(|w| w == data), "leaked data must not appear");
    ...
    let get_own = build_router(env.state.clone())
        ...
        .await
        .expect("oneshot");
    assert_eq!(get_own.status(), StatusCode::OK, "A must read from A's own workspace");
}
...
async fn authz_bypass_cross_workspace_write_denied() {
    ...
    let resp = build_router(env.state.clone())
        ...
        .await
        .expect("oneshot");
    assert_eq!(resp.status(), StatusCode::FORBIDDEN, "A must not write to B's workspace");
    ...
    let resp_own = build_router(env.state.clone())
        ...
        .await
        .expect("oneshot");
    assert_eq!(resp_own.status(), StatusCode::CREATED, "A must write to A's own workspace");
}
...
async fn authz_bypass_under_scoped_token_denied_on_write() {
    ...
    let get_resp = build_router(env.state.clone())
        ...
        .await
        .expect("oneshot");
    assert_eq!(get_resp.status(), StatusCode::OK, "read-only token must be allowed on GET");
}
...
async fn path_traversal_dotdot_in_hash_param_is_bad_request() {
    ...
    let req = Request::builder()
        ...
        .body(Body::empty())
        .expect("request");
    let resp = build_router(env.state.clone()).oneshot(req).await.expect("oneshot");
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST, "../ in hash must be 400 BadRequest");
...
async fn path_traversal_nul_in_hash_param_is_bad_request() {
    ...
    let req = Request::builder()
        ...
        .expect("request");
    let resp = build_router(env.state.clone()).oneshot(req).await.expect("oneshot");
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST, "NUL in hash must be 400 BadRequest");
}
...
async fn path_traversal_dotdot_in_workspace_query_param_denied() {
    ...
    let direct = build_router(env.state.clone())
        ...
        .await
        .expect("oneshot");
    assert_eq!(direct.status(), StatusCode::OK, "direct workspace name must succeed");
}
...
async fn hash_spoof_mismatched_blake3_rejected() {
    ...
    let put = build_router(env.state.clone())
        .oneshot(put_blob_req(&claimed_hex, tampered_data.to_vec(), &tok, "ws"))
        .await
        .expect("oneshot");
    assert_eq!(
        put.status(),
    ...
    assert_eq!(got.as_ref(), real_data, "retrieved bytes must be byte-identical to the original");
}
...
async fn acl_escape_read_only_grant_permits_read_denies_write() {
    ...
    assert_eq!(got.as_ref(), data, "B's GET body must be byte-identical to the original");
    ...
    let put_b = build_router(env.state.clone())
        .oneshot(put_blob_req(&escalation_hex, escalation_data.to_vec(), &tok_b, "shared-ws"))
        .await
        .expect("oneshot");
    assert_eq!(
        put_b.status(),
...
async fn acl_escape_revoked_grant_denied() {
    ...
    let (_, grant_id_b, tok_b) =
        add_user_to_workspace(&env.state.db, "rev-ws", Permission::Read);
