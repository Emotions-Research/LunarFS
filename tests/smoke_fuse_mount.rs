...
fn fuse_t_mount_hydration_and_concurrent_read_stress() {
    ...
    for i in 0u8..2 {
        ...
        let content: Vec<u8> = (0u16..8192)
            .map(|j| (j as u8).wrapping_add(i) % 251)
            .collect();
    ...
    assert!(
        ready,
        "smoke_fuse_mount: mount never became ready within 30s"
    );
    ...
    eprintln!(
        "smoke_fuse_mount: {} files hydrated correctly",
        fixture.len()
    );
    ...
    assert_eq!(
        errors, 0,
        "concurrent-read stress must have zero read errors"
    );
    assert!(
        survived,
        "mount must still serve reads after the stress run"
    );
