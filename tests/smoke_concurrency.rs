...
fn run_agent_thread(
    ...
    for iter in 0..n_iters {
        store
            ...
            .unwrap_or_else(|e| panic!("agent {} capture_write private at iter {}: {}", agent, iter, e));
        ...
        store
            ...
            .unwrap_or_else(|e| panic!("agent {} capture_write iter at iter {}: {}", agent, iter, e));

        if deletes_shared {
            store
                .capture_delete(agent, WS, SHARED_PATH)
                .unwrap_or_else(|e| panic!("agent {} capture_delete at iter {}: {}", agent, iter, e));
        }
...
fn assert_post_run_isolation(store: &OverlayStore, agents: &[(AgentId, bool)]) {
    assert!(agents.len() <= MAX_AGENTS, "agent count must not exceed cap");
    ...
    for &(agent_a, deletes_a) in agents {
        ...
        let shared = store.resolve(agent_a, SHARED_PATH).expect("resolve shared path");
        if deletes_a {
            assert!(
...
fn concurrent_agent_isolation() {
    ...
    for (handle, &(agent, _)) in handles.into_iter().zip(agents.iter()) {
        handle.join().unwrap_or_else(|_| panic!("thread for agent {} panicked", agent));
    }
