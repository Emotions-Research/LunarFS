...
use devdropbox::config::{config_path_with_env, load_config_from_path, resolve_mode, Config, Mode};
use devdropbox::store::InMemoryWorkspaceStore;
use devdropbox::workspace::{
    create_workspace, destroy_workspace, FakeClock, InMemoryBackend, OverlayBackend, WorkspaceSpec,
    WsId,
};
...
fn run_lifecycle_for_mode(mode: Mode, spy: &CloudSpy) -> anyhow::Result<()> {
    ...
    match mode {
        Mode::Local => {
            ...
            let read = backend
                .read(&id, "hello.txt")
                .expect("overlay read must not error");
            assert_eq!(
                read.as_deref(),
            ...
            assert!(
                !backend
                    .exists(&id)
                    .expect("exists after destroy must not error"),
...
fn mode_local_when_only_org_is_set() {
    ...
    assert_eq!(
        resolve_mode(&cfg),
        Mode::Local,
        "org-only config must resolve to Local"
    );
}
...
fn mode_cloud_when_server_is_configured() {
    ...
    assert_eq!(
        resolve_mode(&cfg),
        Mode::Cloud,
        "config with server must resolve to Cloud"
    );
}
...
fn local_mode_lifecycle_zero_cloud_invocations() {
    ...
    run_lifecycle_for_mode(Mode::Local, &spy).expect("LOCAL lifecycle must succeed");
...
fn local_cow_fork_is_o1_zero_bytes_copied() {
    ...
    create_workspace(&backend, &store, &clock, id.clone(), spec).expect("O(1) fork must succeed");
    ...
    assert!(
        absent.is_none(),
        "un-written path must return None after O(1) fork"
    );
