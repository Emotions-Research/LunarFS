...
fn scenario1_disjoint_changes_auto_merge() {
    ...
    assert!(result.conflicts.is_empty(), "disjoint changes must produce zero conflict copies");
    assert_eq!(
    ...
    // No spurious keys.
    assert_eq!(result.merged.len(), 2, "merged must contain exactly 2 entries");
}
...
fn scenario2_edit_vs_edit_keeps_both_versions() {
    ...
    assert_eq!(result.merged.len(), 2, "merged must contain original + conflict copy");
}
...
fn scenario3_winner_edits_local_delete_ignored() {
    ...
    assert!(result.conflicts.is_empty(), "no conflict copy when edit takes priority over delete");
}
...
fn scenario4_racing_cas_advances_converge() {
    ...
    let final_snap = snap_store.fetch(&final_root).expect("final snapshot must be in store");
    assert_eq!(
...
fn isolated_workspace_cas_always_succeeds_no_conflicts() {
    ...
    assert!(matches!(r1, CasResult::Committed { .. }), "isolated: first write must succeed");

    // Sequential advances (single writer): always provide the correct expected_old.
    let r2 = ref_store.advance("isolated", Some("v1"), "v2");
    assert!(matches!(r2, CasResult::Committed { .. }), "isolated: second advance must succeed");

    let r3 = ref_store.advance("isolated", Some("v2"), "v3");
    assert!(matches!(r3, CasResult::Committed { .. }), "isolated: third advance must succeed");
