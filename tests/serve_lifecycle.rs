...
async fn authorized_fork_appears_in_list() {
    ...
    let fork_resp = build_router(env.state.clone())
        ...
        .await
        .expect("oneshot fork");
    assert_eq!(fork_resp.status(), StatusCode::CREATED, "fork must return 201");
    let fork_body = body_json(fork_resp).await;
    let ws_id = fork_body["id"].as_str().expect("id field must be a string").to_string();
    ...
    let workspaces = list_body["workspaces"].as_array().expect("workspaces must be array");
    assert_eq!(workspaces.len(), 1, "list must contain exactly one workspace");
    assert_eq!(workspaces[0]["id"].as_str().unwrap(), ws_id, "listed id must match forked id");
    assert_eq!(
...
async fn unauthorized_path_rejected() {
    ...
    let fork_resp = build_router(env.state.clone())
        .oneshot(fork_req(serde_json::json!({"from_base": "main"}), &caller_tok))
        .await
        .expect("oneshot fork");
    assert_eq!(fork_resp.status(), StatusCode::FORBIDDEN, "unauthorized fork must be 403");
    ...
    let list_body = body_json(list_resp).await;
    let workspaces = list_body["workspaces"].as_array().expect("workspaces must be array");
    assert!(workspaces.is_empty(), "unauthorized caller must see an empty list");
}
...
async fn delete_removes_and_destroys() {
    ...
    assert_eq!(del_resp.status(), StatusCode::OK, "first delete must return 200");
    ...
    assert_eq!(del2_resp.status(), StatusCode::NOT_FOUND, "second delete must return 404");
}
...
async fn ttl_round_trips() {
    ...
    let ephemeral_resp = build_router(env.state.clone())
        .oneshot(fork_req(serde_json::json!({"from_base": "main", "ttl_secs": 7200}), &tok))
        .await
        .expect("oneshot ephemeral fork");
    assert_eq!(ephemeral_resp.status(), StatusCode::CREATED, "ephemeral fork must return 201");
    ...
    assert_eq!(persistent_resp.status(), StatusCode::CREATED, "persistent fork must return 201");
    ...
    let workspaces = body_json(list_resp).await["workspaces"]
        ...
        .expect("workspaces array")
        .clone();
    assert_eq!(workspaces.len(), 2, "list must contain exactly two workspaces");
    ...
    assert_eq!(
        ephemeral_ws["ttl_secs"].as_u64().expect("ttl_secs must be u64"),
        7200,
