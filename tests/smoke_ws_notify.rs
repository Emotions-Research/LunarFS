...
use devdropbox::live_sync::{
    ClientReconciler, LiveSnapshot, MemServerApi, NotifyChannel, ServerApi, SocketNotifyChannel,
};
...
fn smoke_ws_notify_socket_roundtrip() {
    ...
    let addr = server_channel
        .local_addr()
        .expect("server must have a local addr");
    ...
    api.seed_snapshot(make_snapshot(
        "s1",
        &[("alpha.txt", "ws-h1"), ("beta.txt", "ws-h2")],
    ));
    ...
    let reconciler = Arc::new(ClientReconciler::new(api.clone() as Arc<dyn ServerApi>));
    ...
    for i in 0..MAX_WAIT_ITERS {
        ...
        assert!(
            i < MAX_WAIT_ITERS - 1,
            "client must reconcile S1 within 1s over TCP"
        );
        std::thread::sleep(std::time::Duration::from_millis(WAIT_STEP_MS));
    }
    assert!(
        received_s1.load(Ordering::Acquire),
        "client must have received S1 event"
    );

    let mount1 = reconciler.current_mount();
    assert_eq!(
        mount1.snapshot_id.as_deref(),
        Some("s1"),
        "mount must be at S1"
    );
    assert!(
        mount1.entries.contains_key("alpha.txt"),
        "S1 mount must contain alpha.txt"
    );
    assert!(
        mount1.entries.contains_key("beta.txt"),
        "S1 mount must contain beta.txt"
    );
    assert_eq!(
        mount1.entries.get("alpha.txt").map(|s| s.as_str()),
        Some("ws-h1")
    );
    ...
    for i in 0..MAX_WAIT_ITERS {
        ...
        assert!(
            i < MAX_WAIT_ITERS - 1,
            "client must reconcile S2 within 1s over TCP"
        );
        std::thread::sleep(std::time::Duration::from_millis(WAIT_STEP_MS));
    }
    assert!(
        received_s2.load(Ordering::Acquire),
        "client must have received S2 event"
    );

    let mount2 = reconciler.current_mount();
    assert_eq!(
        mount2.snapshot_id.as_deref(),
        Some("s2"),
        "mount must be at S2"
    );
    assert_eq!(
        mount2.entries, s2_entries,
        "S2 mount entries must match seeded snapshot"
    );
    assert!(
        !mount2.entries.contains_key("beta.txt"),
        "beta.txt must not appear in S2"
    );
