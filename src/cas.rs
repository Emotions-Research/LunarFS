...
impl Store for FsStore {
    fn put(&self, data: &[u8]) -> io::Result<Hash> {
        ...
        let parent = path
            .parent()
            .ok_or_else(|| io::Error::other("blob path has no parent directory"))?;
...
impl HydratingStore {
    pub fn new(local: Arc<dyn Store>, remote: Arc<dyn BlobSource>) -> Self {
        assert!(
            Arc::strong_count(&local) >= 1,
            "local store ref must be live"
        );
        assert!(
            Arc::strong_count(&remote) >= 1,
            "remote source ref must be live"
        );
        Self { local, remote }
    }
}
