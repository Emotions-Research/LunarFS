...
fn build_stub_url(
    ...
    format!("stub+local://{}/{}/{}/{}/{}", base_hex, expires_at, op_str(op), sig_hex, pct)
}
...
impl Presigner for LocalStubPresigner {
    fn presign(
        ...
        Ok(PresignedUrl { url, method: method.to_string(), expires_at })
    }
}
...
pub fn resolve_stub(url: &str, now_secs: i64) -> Result<ResolvedStub, PresignError> {
    ...
    let rest = url.strip_prefix("stub+local://").ok_or_else(|| {
        PresignError::Malformed("missing stub+local:// scheme".to_string())
    })?;
    ...
    Ok(ResolvedStub { fs_path, op, expires_at })
}
...
mod tests {
    ...
    const OBJECT_PATH: &str = "blobs/aa/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";

    #[test]
    fn stub_presign_resolve_roundtrip() {
        let dir = tempfile::tempdir().expect("tempdir");
        let p = LocalStubPresigner::new(dir.path());
        let signed = p.presign(PresignOp::Put, OBJECT_PATH, 300, 1000).expect("presign");
    ...
    async fn stub_put_then_fetch_byte_correct() {
        ...
        let put_url = p.presign(PresignOp::Put, OBJECT_PATH, 300, now).expect("presign put");
        let get_url = p.presign(PresignOp::Get, OBJECT_PATH, 300, now).expect("presign get");

        let client = reqwest::Client::new();
        put_presigned(&client, &put_url.url, data.to_vec(), now).await.expect("put_presigned");

        // Verify the file actually exists at the resolved FS path.
        let resolved = resolve_stub(&put_url.url, now).expect("resolve");
        assert!(resolved.fs_path.exists(), "blob must exist on disk at resolved path");

        let got = fetch_presigned(&client, &get_url.url, now).await.expect("fetch_presigned");
    ...
    fn expired_stub_is_rejected() {
        let dir = tempfile::tempdir().expect("tempdir");
        let p = LocalStubPresigner::new(dir.path());
        let signed = p.presign(PresignOp::Get, OBJECT_PATH, 10, 1000).expect("presign");
    ...
    fn malformed_url_is_rejected() {
        ...
        assert!(matches!(err, PresignError::Malformed(_)), "expected Malformed, got: {}", err);
    }
}
