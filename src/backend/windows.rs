...
use windows::Win32::Storage::ProjectedFileSystem::{
    PrjFillDirEntryBuffer, PrjMarkDirectoryAsPlaceholder, PrjStartVirtualizing, PrjWriteFileData,
    PrjWritePlaceholderInfo, PRJ_CALLBACKS, PRJ_CALLBACK_DATA, PRJ_DIR_ENTRY_BUFFER_HANDLE,
    PRJ_FILE_BASIC_INFO, PRJ_NOTIFICATION, PRJ_NOTIFICATION_FILE_HANDLE_CLOSED_FILE_DELETED,
...
unsafe extern "system" fn get_dir_enum(
    ...
    while *cursor < entries.len() {
        ...
        info.FileAttributes = if entry.is_dir {
...
unsafe extern "system" fn get_placeholder_info(callback_data: *const PRJ_CALLBACK_DATA) -> HRESULT {
    ...
    info.FileBasicInfo.FileAttributes = if attrs.is_dir {
        FILE_ATTRIBUTE_DIRECTORY
    } else {
        FILE_ATTRIBUTE_NORMAL
    };
...
pub fn mount_windows(repo: &Path, mountpoint: &Path) -> anyhow::Result<()> {
    assert!(
        repo.is_dir(),
        "repo must be an existing directory before calling mount_windows"
    );
    assert!(
        mountpoint.is_dir(),
    ...
    let mut gb = [0u8; 16];
    getrandom::getrandom(&mut gb).map_err(|e| anyhow::anyhow!("getrandom failed: {}", e))?;
    ...
    if let Err(e) = unsafe {
        PrjMarkDirectoryAsPlaceholder(root, PCWSTR(std::ptr::null::<u16>()), None, &instance_guid)
    } {
    ...
    let _virt_ctx =
        unsafe { PrjStartVirtualizing(root, &callbacks, Some(ctx_ptr as *const c_void), None) }
