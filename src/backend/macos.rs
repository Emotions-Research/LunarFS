...
impl MacFuseSession {
    ...
    pub fn build(core: Core, mountpoint: &Path) -> Result<Self> {
        ...
        if !mountpoint.is_dir() {
            anyhow::bail!("mountpoint must be an existing directory: {}", mountpoint.display());
        }
        ...
        assert!(ops.getattr.is_some(), "getattr callback must be wired in ops");
        Ok(Self { mountpoint: mountpoint.to_path_buf(), ops, fs_data })
    }
    ...
    pub fn with_overlay(
        ...
        let mut data = Arc::try_unwrap(self.fs_data)
            .unwrap_or_else(|_| panic!("MacFuseSession must be the sole Arc owner when calling with_overlay"));
        data.overlay = Some(overlay);
        data.agent = agent;
        data.workspace = workspace;
        Self { mountpoint, ops, fs_data: Arc::new(data) }
    }
    ...
    pub fn with_acl(self, entries: Vec<AclEntry>) -> Self {
        ...
        let mountpoint = self.mountpoint;
        let mut data = Arc::try_unwrap(self.fs_data)
            .unwrap_or_else(|_| panic!("MacFuseSession must be the sole Arc owner when calling with_acl"));
        data.acl = entries;
        Self { mountpoint, ops, fs_data: Arc::new(data) }
    }
    ...
    pub fn run_with<F: FnOnce(&mut Self) -> Result<()>>(mut self, enter_loop: F) -> Result<()> {
        ...
        assert!(self.ops.getattr.is_some(), "ops must have getattr wired before running");
        enter_loop(&mut self)
    }
}
...
pub fn mount_macos(repo: &Path, mountpoint: &Path) -> Result<()> {
    assert!(repo.is_dir(), "repo must be a directory before calling mount_macos");
    assert!(mountpoint.is_dir(), "mountpoint must be a directory before calling mount_macos");
...
mod tests {
    ...
    fn make_test_core(repo: &Path) -> Core {
        ...
        Core { store: Box::new(store), index }
    }
    ...
    fn macos_session_builds_and_no_op_loop_succeeds() {
        ...
        session.run_with(|_s| Ok(())).expect("no-op run_with must succeed");
    }
