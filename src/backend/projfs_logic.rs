...
pub fn placeholder_from_attr(attr: &Attributes) -> PlaceholderMeta {
    assert!(
        ...
        "file size must fit in a LARGE_INTEGER (Windows i64): size={}",
        attr.size
    );
    match attr.kind {
...
pub fn placeholder_info(
    ...
    Ok(ProjFsAttrs {
        is_dir: attrs.kind == NodeKind::Dir,
        size,
    })
}
...
pub fn read_file_data(
    ...
    resolve_read(
        index, store, overlay, acl, agent, principal, now, path, offset, size_u32,
    )
}
...
pub fn apply_write_notification(
    ...
    assert!(
        path.len() <= 4096,
        "path must not exceed 4096 bytes: path={}",
        path
    );
    assert!(
        principal.len() <= 256,
    ...
    match op {
        ProjFsWriteOp::Write => {
            let data =
                bytes.ok_or_else(|| FsError::IoError("write notification missing bytes".into()))?;
            route_write(store, overlay, agent, workspace, path, data)
                .map(|_| ())
                .map_err(|e| FsError::IoError(e.to_string()))
        }
        ProjFsWriteOp::Delete => route_delete(overlay, agent, workspace, path)
            .map_err(|e| FsError::IoError(e.to_string())),
    }
}
...
mod tests {
    ...
    fn placeholder_file_preserves_size_and_sets_normal_bit() {
        ...
        assert_eq!(
            meta.file_size, 1234,
            "file size must be preserved from input"
        );
        assert!(
            !meta.is_directory,
            "file node must not be marked as directory"
        );
        assert_ne!(
    ...
    fn placeholder_dir_sets_directory_bit_and_zero_size() {
        ...
        assert!(
            meta.is_directory,
            "directory node must be marked as directory"
        );
    ...
    mod read_path_tests {
        ...
        fn placeholder_info_overlay_size_wins_over_base() {
            ...
            assert_eq!(
                ...
                "overlay size must win"
            );
            assert_ne!(
                attrs.size,
                b"hi".len() as u64,
                "must not return stale base size"
            );
        }
        ...
        fn read_file_data_offset_plus_length_past_eof() {
            ...
            assert_eq!(
                got, b"de",
                "past-EOF window must be clamped to available tail"
            );
        }
        ...
        fn read_file_data_zero_length_returns_empty() {
            ...
            assert!(
                got.is_empty(),
                "zero-length request must return empty slice"
            );
        }
        ...
        fn enumerate_dir_returns_sorted_entries_with_real_sizes() {
            ...
            assert_eq!(
                entries[0].size,
                b"alpha".len() as u64,
                "alpha size must equal blob length"
            );
            assert_eq!(
                entries[1].size,
                b"beta".len() as u64,
                "beta size must equal blob length"
            );
            assert_eq!(
                entries[2].size,
                b"gamma".len() as u64,
                "gamma size must equal blob length"
            );
        }
        ...
        fn enumerate_dir_missing_path_is_not_found() {
            ...
            assert!(
                matches!(err, FsError::NotFound),
                "missing dir must return NotFound"
            );
        }
        ...
        fn enumerate_dir_on_file_is_not_a_dir() {
            ...
            .unwrap_err();
            assert!(
                matches!(err, FsError::NotADir),
                "calling on a file must return NotADir"
            );
        }

        #[test]
        fn enumerate_dir_subdirectory() {
            use crate::tree::{TreeEntry as TE, MODE_DIR};
    ...
    mod write_path_tests {
        ...
        fn write_capture_then_readback() {
            ...
            assert!(
                result.is_ok(),
                "write capture must succeed with no ACL restriction"
            );
        ...
        fn delete_tombstone_hides_base() {
            ...
            assert!(
                result.is_ok(),
                "tombstone delete must succeed with no ACL restriction"
            );
            ...
            assert!(
                ...
                "tombstoned file must return NotFound on read; got {:?}",
                read_err
            );
            ...
            assert!(
                ...
                "tombstoned file must return NotFound on placeholder_info; got {:?}",
                attr_err
            );
        }
        ...
        fn acl_deny_blocks_write() {
            ...
            assert!(
                ...
                "ACL deny must return AccessDenied; got {:?}",
                result
            );
        ...
        fn fs_error_to_win32_mappings() {
            ...
            assert_eq!(
                fs_error_to_win32(&FsError::IoError("x".into())),
                0x8007001F,
