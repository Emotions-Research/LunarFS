...
impl WorkspaceStore for InMemoryWorkspaceStore {
    ...
    fn list_all(&self) -> Result<Vec<Workspace>> {
        ...
        assert!(
            order.len() <= 1_000_000,
            "workspace list exceeds sanity cap"
        );
        let result = order
            .iter()
...
impl SqliteWorkspaceStore {
    ...
    pub fn open(conn: Connection) -> Result<Self> {
        ...
        // Upgrade path: add root column to databases created before this migration.
        // Ignore the error when the column already exists.
        let _ = conn.execute("ALTER TABLE local_workspaces ADD COLUMN root TEXT", []);
        Ok(Self {
            conn: Mutex::new(conn),
        })
    }
...
impl WorkspaceStore for SqliteWorkspaceStore {
    fn save(&self, ws: &Workspace) -> Result<()> {
        assert!(!ws.id.0.is_empty(), "WsId must not be empty in save");
        assert!(
            !ws.base_ref.is_empty(),
            "base_ref must not be empty in save"
        );
        let meta = metadata_to_json(&ws.metadata)?;
        let conn = self.conn.lock().expect("store conn lock poisoned");
        conn.execute(
            "INSERT INTO local_workspaces (id, label, base_ref, ttl_secs, created_at, ephemeral, metadata, root)
    ...
    fn get(&self, id: &WsId) -> Result<Option<Workspace>> {
        ...
        let result: rusqlite::Result<Workspace> = conn.query_row(
            "SELECT id, label, base_ref, ttl_secs, created_at, ephemeral, metadata, root
...
mod tests {
    ...
    fn sqlite_store_roundtrip() {
        ...
        store.save(&ws).expect("save");
        let got = store
            .get(&WsId("ws-sqlite-001".to_string()))
            .expect("get")
            .expect("Some");
