...
fn resolve_path(index: &dyn IndexSeam, path: &str) -> Result<NodeMeta, FsError> {
    ...
    if path.is_empty() {
        return Ok(NodeMeta { kind: NodeKind::Dir, size: 0, mode: 0o040755, hash: None });
    }
    ...
    if exists {
        Ok(NodeMeta { kind: NodeKind::Dir, size: 0, mode: 0o040755, hash: None })
    } else {
...
pub fn attr_for(index: &dyn IndexSeam, path: &str) -> Result<Attributes, FsError> {
    ...
    Ok(Attributes { kind: meta.kind, size: meta.size, mode: meta.mode })
}
...
pub fn read_slice(
    ...
    assert!(path.len() <= 4096, "path must not exceed 4096 bytes");
    assert!(size as u64 <= 128 * 1024 * 1024, "read size must be <= 128 MiB");

    let meta = resolve_path(index, path)?;
    if meta.kind == NodeKind::Dir {
        return Err(FsError::IsADir);
    }
    let hash = meta.hash.ok_or_else(|| FsError::IoError(format!("no hash for {}", path)))?;
    let bytes = store
        .hydrate(&hash)
...
pub fn read_dir(index: &dyn IndexSeam, path: &str) -> Result<Vec<DirEntry>, FsError> {
    ...
    let mut entries: Vec<DirEntry> = vec![
        DirEntry { name: ".".to_owned(), kind: NodeKind::Dir },
        DirEntry { name: "..".to_owned(), kind: NodeKind::Dir },
    ];
    ...
    for file_path in index.file_paths() {
        ...
        let kind = if rest.contains('/') { NodeKind::Dir } else { NodeKind::File };
        entries.push(DirEntry { name: component.to_owned(), kind });
    }
...
mod tests {
    ...
    impl FakeIndex {
        ...
        fn add(&mut self, path: &str, content: &[u8], mode: u32) -> Hash {
            ...
            self.files.insert(path.to_owned(), (content.len() as u64, mode, h));
            h
        }
    }
    ...
    impl FakeStore {
        fn new(counter: Arc<AtomicU32>) -> Self {
            Self { blobs: HashMap::new(), hydrate_count: counter }
        }
        fn insert(&mut self, hash: Hash, data: Vec<u8>) { self.blobs.insert(hash, data); }
    }
    ...
    fn read_slice_hydrate_count_exactly_one() {
        ...
        assert_eq!(got, content);
        assert_eq!(counter.load(Ordering::Relaxed), 1, "hydrate must be called exactly once");
    }
    ...
    fn read_dir_nested() {
        ...
        let names = entry_names(&entries);
        assert!(names.contains(&"main.rs".to_owned()), "direct file child must appear");
