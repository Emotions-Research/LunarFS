...
fn check_acl(acl: &[AclEntry], path: &str, principal: &str, now: i64) -> Result<(), FsError> {
    assert!(acl.len() <= 1_000_000, "acl slice exceeds safe cap of 1M entries");
...
pub fn resolve_read(
    ...
    check_acl(acl, path, principal, now)?;
    if let Some(ov) = overlay {
        return super::route_read(store, index, ov, agent, path, offset as usize, size as usize)
...
pub fn resolve_size(
    ...
    if let Some(ov) = overlay {
        match ov.resolve(agent, path).map_err(|e| FsError::IoError(e.to_string()))? {
...
mod tests {
    ...
    fn resolve_tombstone_is_enoent() {
        ...
        .unwrap_err();
        assert!(matches!(err_read, FsError::NotFound), "tombstone read must be NotFound");
        ...
        .unwrap_err();
        assert!(matches!(err_attr, FsError::NotFound), "tombstone attr must be NotFound");
    ...
    fn resolve_allowed_path() {
        ...
        let attrs =
            resolve_attr(&index, Some(&overlay), &no_acl(), agent, PRINCIPAL, NOW, "pub.txt")
                .unwrap();
    ...
    fn size_overlay_write_wins() {
        ...
        .unwrap();
        assert_eq!(sz, b"longer content".len() as u64, "overlay size must win over base");
    ...
    fn size_tombstone_is_enoent() {
        ...
        .unwrap_err();
        assert!(matches!(err, FsError::NotFound), "tombstone must return NotFound");
    ...
    fn size_acl_deny_is_eacces() {
        ...
        .unwrap_err();
        assert!(matches!(err, FsError::AccessDenied), "ACL deny must return AccessDenied");
    ...
    fn resolve_overlay_acl_interaction() {
        ...
        .unwrap();
        assert_eq!(bytes, b"upper-value", "allowed shadow must return upper bytes");
    }
}
