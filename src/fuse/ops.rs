...
use crate::fuse::resolve::{
    resolve_attr, resolve_open, resolve_read, resolve_readdir, resolve_size,
};
use crate::fuse::translate::{errno_for, FsError, IndexSeam, NodeKind, NodeMeta, StoreSeam};
use crate::fuse_t::{FuseFileInfo, FuseFillDirT, FuseOperations};
...
unsafe fn path_str<'a>(raw: *const c_char) -> Result<&'a str, c_int> {
    ...
    CStr::from_ptr(raw).to_str().map_err(|_| -libc::EINVAL)
}
...
mod tests {
    ...
    fn size_base_file_equals_blob_length() {
        ...
        assert_eq!(
            meta.size, sz,
            "IndexSeam::lookup size must match blob length"
        );
    }
