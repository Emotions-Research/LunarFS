...
pub fn resolve_workspace(
    ...
    let token = env_nonempty(env, ENV_TOKEN)
        ...
        .ok_or_else(|| anyhow::anyhow!("no auth token: run 'lunar login' or set {}", ENV_TOKEN))?;

    assert!(!server.is_empty(), "resolved server must not be empty");
    assert!(
        !workspace.is_empty(),
        "resolved workspace must not be empty"
    );
...
pub fn parse_workspace_segment(name: &str) -> Result<String> {
    ...
    let last = *segments
        .last()
        .expect("non-empty segments vec has a last element");
    if last.is_empty() {
        anyhow::bail!(
            "invalid workspace name {:?}: trailing slash or empty last segment",
            name
        );
    }
    Ok(last.to_string())
