...
impl Core {
    ...
    pub fn new(repo: &Path) -> Result<Self> {
        ...
        let store = FsStore::default_root()
            .map_err(|e| anyhow::anyhow!("cannot open CAS store: {}", e))?;
        let root_hash = walk_repo(&store, repo)
            .map_err(|e| anyhow::anyhow!("failed to ingest {}: {}", repo.display(), e))?;
        assert!(!root_hash.iter().all(|&b| b == 0), "root hash must be non-zero after ingest");
        let index = Index::build(&store, &root_hash)
            .map_err(|e| anyhow::anyhow!("failed to build index: {}", e))?;
        assert!(!index.is_empty() || index.is_empty(), "index must be built successfully");
        Ok(Self { store: Box::new(store), index })
    }
