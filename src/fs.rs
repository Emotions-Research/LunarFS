...
impl ReadCache {
    ...
    pub fn read_path(
        ...
        assert!(size <= 128 * 1024 * 1024, "requested size must be sane (<=128 MiB)");
        ...
        {
            return Err(io::Error::new(io::ErrorKind::PermissionDenied, "access denied by ACL"));
        }
...
mod fuse_impl {
    ...
    impl Filesystem for LunarFs {
        fn lookup(
            ...
            let child_ino = children.iter().find_map(|&ino| {
                self.inodes.get(&ino).filter(|e| e.name == name_str.as_ref()).map(|_| ino)
            });
            match child_ino {
                Some(ino) => {
        ...
        fn read(
            ...
            if let Some(ref ov) = self.overlay.clone() {
                ...
                match self.read_cache.read_path(&path, offset as usize, size as usize) {
                    Ok(Some(data)) => reply.data(&data),
                    Ok(None) => reply.error(2 /* ENOENT */),
                    Err(_) => reply.error(5 /* EIO */),
                }
    ...
    mod tests {
        ...
        fn make_attr_base_file_size() {
            ...
            let sz = resolve_size(
                &index, &store, Some(ov.as_ref()),
                &[], agent, PRINCIPAL, NOW, "greet.txt",
            ).unwrap();
        ...
        fn size_acl_deny_is_access_denied() {
            ...
            let err = resolve_size(
                &index, &store, Some(ov.as_ref()),
                &acl, agent, PRINCIPAL, NOW, "secret.txt",
            ).unwrap_err();
...
mod tests {
    ...
    impl CountingStore {
        fn new() -> (Self, Arc<Mutex<usize>>) {
            ...
            let s = Self { inner: MemStore::new(), get_count: counter.clone() };
            (s, counter)
        }
    }
    ...
    fn read_cache_gate_test() {
        ...
        let root_bytes = serialize_tree(&[
            TreeEntry { mode: MODE_DIR, name: "src".into(), hash: src_tree_h },
        ...
        *counter.lock().unwrap() = 0;
        ...
        let bytes = cache.read_path("src/main.rs", 0, 1024).unwrap().expect("path must exist");
        ...
        let bytes2 = cache.read_path("src/main.rs", 0, 1024).unwrap().expect("path must exist");
        assert_eq!(bytes2, b"fn main() {}");
        assert_eq!(cache.fetch_count, 1, "second read must be served from cache, not re-fetched");

        // Partial read with offset and size.
        let partial = cache.read_path("src/main.rs", 3, 4).unwrap().expect("path exists");
        ...
        let beyond = cache.read_path("src/main.rs", 9999, 10).unwrap().expect("path exists");
        ...
        cache.read_path("README.md", 0, 1024).unwrap().expect("readme must exist");
    ...
    fn make_index_with_file(name: &str, content: &[u8]) -> (MemStore, Index) {
        ...
        let tree_bytes = serialize_tree(&[TreeEntry { mode: MODE_FILE, name: name.to_string(), hash }]);
    ...
    fn read_path_denied_by_acl_returns_permission_denied() {
        ...
        let mut cache =
            ReadCache::new(Box::new(store), index).with_acl(entries, "any".to_string());
        let err = cache.read_path("secret.txt", 0, 1024).expect_err("must be denied");
    ...
    fn read_path_allowed_by_acl_returns_bytes() {
        ...
        let mut cache =
            ReadCache::new(Box::new(store), index).with_acl(entries, "any".to_string());
        let got = cache.read_path("pub.txt", 0, 1024).unwrap().expect("must return bytes");
    ...
    fn read_path_not_in_acl_is_open() {
        ...
        let mut cache =
            ReadCache::new(Box::new(store), index).with_acl(entries, "any".to_string());
        let got = cache.read_path("open.txt", 0, 1024).unwrap().expect("open path must be readable");
    ...
    fn read_path_denied_vs_absent_are_distinct() {
        ...
        let err = cache.read_path("exists.txt", 0, 64).expect_err("must error");
        assert_eq!(err.kind(), std::io::ErrorKind::PermissionDenied);

        // Absent path (no ACL entry): returns Ok(None).
        let absent = cache.read_path("nonexistent.txt", 0, 64).unwrap();
        assert!(absent.is_none(), "absent path must be Ok(None), not an error");
    }
}
