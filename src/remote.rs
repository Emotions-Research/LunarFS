...
// ---------------------------------------------------------------------------
// CAS ref outcome (typed; returned by put_ref_cas_outcome)
// ---------------------------------------------------------------------------

/// Result of a compare-and-swap ref-advance request.
pub enum CasRefOutcome {
    /// The server accepted the update; the ref now points at the new root.
    Committed,
    /// The server rejected the update because its current root did not match
    /// the supplied expected_root. The incoming root was saved under conflict_ref.
    Conflict {
        current_root: Hash,
        conflict_ref: String,
    },
...
impl HttpRemote {
    ...
    pub async fn missing_blobs(
        ...
        anyhow::ensure!(
            status.is_success(),
            "POST /v1/blobs/missing returned {}",
            status
        );
        ...
        for v in arr {
            let hex = v
                .as_str()
                .ok_or_else(|| anyhow::anyhow!("hash entry is not a string"))?;
            result.push(
                hex_to_hash(hex).map_err(|e| anyhow::anyhow!("bad hash in response: {}", e))?,
            );
        }
        Ok(result)
    }
    ...
    pub async fn presign(
        ...
        assert_eq!(
            hash_to_hex(hash).len(),
            64,
            "hash_to_hex must produce 64 chars"
        );
    ...
    /// PUT /v1/ref/:workspace with optional compare-and-swap; returns a typed outcome.
    ///
    /// On HTTP 409, parses `{conflict_ref, current_root}` and returns
    /// `Ok(CasRefOutcome::Conflict { .. })`. On 2xx returns `Ok(CasRefOutcome::Committed)`.
    /// Any other non-success status is an `Err`. Missing or unparseable fields in a 409
    /// body are also surfaced as `Err`.
    pub async fn put_ref_cas_outcome(
        &self,
        workspace: &str,
        root: &Hash,
        expected_root: Option<&Hash>,
    ) -> anyhow::Result<CasRefOutcome> {
        assert!(!workspace.is_empty(), "workspace must not be empty");
        ...
        if status == reqwest::StatusCode::CONFLICT {
            ...
            let conflict_ref = payload["conflict_ref"]
                .as_str()
                .ok_or_else(|| anyhow::anyhow!("conflict response missing 'conflict_ref'"))?
                .to_string();
            let current_root_hex = payload["current_root"]
                .as_str()
                .ok_or_else(|| anyhow::anyhow!("conflict response missing 'current_root'"))?;
            let current_root = hex_to_hash(current_root_hex)
                .map_err(|e| anyhow::anyhow!("bad current_root in conflict response: {}", e))?;
            return Ok(CasRefOutcome::Conflict {
                current_root,
                conflict_ref,
            });
        }
        ...
        Ok(CasRefOutcome::Committed)
    }

    /// PUT /v1/ref/:workspace with optional compare-and-swap.
    ///
    /// Delegates to `put_ref_cas_outcome`; maps `Conflict` to a human-readable
    /// `anyhow::bail!` so `lunar push` and existing callers remain unchanged.
...
mod tests {
    ...
    fn read_head_fresh_workspace_is_none() {
        ...
        let result = remote
            .read_head()
            .expect("read_head must not error on fresh workspace");
    ...
    fn init_head_then_read_head_round_trips() {
        ...
        let got = remote
            .read_head()
            .unwrap()
            .expect("read_head must return Some");
    ...
    fn update_head_stale_version_is_conflict() {
        ...
        let stale = UpdateVersion {
            e_tag: Some("wrong-etag-xyz".to_string()),
            version: None,
        };
        let err = remote
            .update_head(&r2, &stale)
    ...
    fn update_head_current_version_succeeds() {
        ...
        let ptr2 = remote
            .update_head(&r2, &ptr1.version)
            .expect("CAS with current version must succeed");
    ...
    fn presign_url_get_no_workspace() {
        ...
        assert_eq!(
            url,
            format!("http://localhost:9000/v1/blob/{}/presign?op=get", hex)
        );
    }
    ...
    fn presign_url_put_with_workspace() {
        ...
        assert_eq!(
            url,
            format!(
                "http://localhost:9000/v1/blob/{}/presign?op=put&workspace=my-ws",
                hex
            )
        );
    }
    ...
    async fn put_blob_fallback_returns_err_on_unreachable_server() {
        ...
        assert!(
            result.is_err(),
            "put_blob against unreachable port must return Err"
        );
    }
    ...
    async fn get_blob_fallback_returns_err_on_unreachable_server() {
        ...
        assert!(
            result.is_err(),
            "get_blob against unreachable port must return Err"
        );
    }
}
