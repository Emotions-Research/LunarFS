...
// mount-nfs takes priority over all other backends when enabled.
...
#[cfg(all(not(feature = "mount-nfs"), target_os = "windows", feature = "projfs"))]
fn dispatch_mount(repo: &Path, mountpoint: &Path) -> Result<()> {
...
#[cfg(not(any(
    feature = "mount-nfs",
    all(target_os = "linux", feature = "fuser"),
    all(target_os = "macos", feature = "fuse"),
...
mod tests {
    ...
    fn selected_backend_matches_target_os() {
        ...
        #[cfg(target_os = "windows")]
        assert_eq!(b, Backend::Windows, "Windows build must return Backend::Windows");
    }
    ...
    fn backend_enum_is_eq_and_clone() {
        ...
        assert_ne!(Backend::MacOs, Backend::Windows, "variants must be distinct");
        assert_ne!(Backend::Linux, Backend::Windows, "variants must be distinct");
    }
}
