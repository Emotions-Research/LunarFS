...
pub fn route_read(
    ...
    assert!(size <= 128 * 1024 * 1024, "requested size must be <= 128 MiB");
...
mod tests {
    ...
    fn delete_tombstone() {
        ...
        let result =
            route_read(&store, &index, &overlay, agent_a, "del.txt", 0, 1024).unwrap();
        assert!(result.is_none(), "tombstoned path must return None for deleting agent");
    ...
    fn write_clears_tombstone() {
        ...
        assert!(!is_tombstoned(&overlay, agent, "t.txt"), "tombstone must be cleared by write");
    ...
    fn absent_path_returns_none() {
        ...
        let result =
            route_read(&store, &index, &overlay, agent, "no_such.txt", 0, 1024).unwrap();
    ...
    fn smoke_concurrent_agents() {
        ...
        let agents: Vec<AgentId> =
            (0..N).map(|_| overlay.fork(WS_SMOKE).unwrap()).collect();
