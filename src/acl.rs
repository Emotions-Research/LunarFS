...
pub fn decide(
    ...
    assert!(entries.len() <= 1_000_000, "entries slice exceeds safe cap of 1M");
...
impl AclStore {
    ...
    pub fn insert(&self, entry: &AclEntry) -> Result<()> {
        assert!(entry.path_prefix.len() <= 4096, "path_prefix must not exceed 4096 bytes");
        let perm_str = permission_to_str(entry.permission);
        let conn = self.conn.lock().expect("acl conn lock poisoned");
        conn.execute(
            "INSERT INTO acl (path_prefix, principal, permission, expires_at)
             VALUES (?1, ?2, ?3, ?4)",
            params![entry.path_prefix, entry.principal, perm_str, entry.expires_at],
        )?;
        Ok(())
    }
    ...
    pub fn load_all(&self) -> Result<Vec<AclEntry>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT path_prefix, principal, permission, expires_at FROM acl",
        )?;
        ...
        for row in rows {
            ...
            entries.push(AclEntry { path_prefix, principal, permission, expires_at });
        }
        Ok(entries)
    }
}
...
mod tests {
    ...
    fn longest_prefix_wins() {
        ...
        assert_eq!(
            decide(&entries, "docs/internal/design.md", "*", 0, Permission::Read),
            AclDecision::Deny
        );
    }
    ...
    fn acl_store_roundtrip() {
        ...
        let doc_entry = loaded.iter().find(|e| e.path_prefix == "docs").expect("docs entry");
        assert_eq!(doc_entry.permission, Permission::Read);
        assert_eq!(doc_entry.expires_at, None);

        let sec_entry = loaded.iter().find(|e| e.path_prefix == "secret").expect("secret entry");
