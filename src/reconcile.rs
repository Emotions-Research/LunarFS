...
impl MemSnapshotStore {
    ...
    pub fn seed(&self, root: &str, snap: Snapshot) {
        ...
        self.snaps.lock().expect("snaps lock poisoned").insert(root.to_string(), snap);
    }
}
...
impl SnapshotStore for MemSnapshotStore {
    fn fetch(&self, root: &str) -> Option<Snapshot> {
        assert!(!root.is_empty(), "root must not be empty");
        self.snaps.lock().expect("snaps lock poisoned").get(root).cloned()
    }
    ...
    fn commit(&self, snapshot: Snapshot) -> String {
        ...
        self.snaps.lock().expect("snaps lock poisoned").insert(id.clone(), snapshot);
        id
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ReconcileError::MaxRetriesExceeded { attempts } => {
                write!(f, "reconcile: max retries ({}) exceeded without convergence", attempts)
            }
        }
    }
}
...
pub fn reconcile(
    ...
    assert!(!conflict_suffix.is_empty(), "conflict_suffix must not be empty");
    ...
    let mut winner_root = ref_store.read(ref_name).ok_or(ReconcileError::RefNotFound)?;
    ...
    Err(ReconcileError::MaxRetriesExceeded { attempts: max_retries })
}
...
mod tests {
    ...
    fn snap(entries: &[(&str, &str)]) -> Snapshot {
        entries.iter().map(|(p, c)| (p.to_string(), present(c))).collect()
    }
    ...
    fn reconcile_merges_disjoint_changes() {
        ...
        let final_snap = snap_store.fetch(&final_root).expect("final snapshot must exist");
    ...
    fn reconcile_fails_on_too_many_retries() {
        ...
        impl RefStore for NeverCasStore {
            fn advance(&self, _: &str, _: Option<&str>, _: &str) -> CasResult {
                CasResult::Mismatch { current: "adversarial-v1".to_string() }
            }
        ...
        assert!(
            matches!(result, Err(ReconcileError::MaxRetriesExceeded { attempts: 3 })),
