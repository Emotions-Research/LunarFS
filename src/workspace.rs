...
pub struct Workspace {
    ...
    pub ephemeral: bool,
    /// The current root tree hash (64-char hex) for this workspace, if recorded.
    /// Set by push/commit operations; None until the first root is recorded.
...
impl LocalFsBackend {
    pub fn new(root: std::path::PathBuf) -> Self {
        assert!(
            ...
            "LocalFsBackend root must not be empty"
        );
        Self { root }
    }
...
pub struct WorkspaceSpec {
    ...
    pub ttl: Option<Duration>,
    /// Optional initial root hash (64-char hex). Typically None at fork time.
    pub root: Option<String>,
}
...
pub fn destroy_workspace(
    ...
    assert!(
        !backend.exists(id)?,
        "backend must not have overlay after destroy"
    );
    Ok(())
}
...
mod tests {
    ...
    fn test_isolation() {
        ...
        backend
            .write(&id_a, "file.txt", b"hello from A")
            .expect("write to A");
        ...
        backend
            .write(&id_b, "other.txt", b"hello from B")
            .expect("write to B");

        // A does not see B's write.
        let a_view2 = backend
            .read(&id_a, "other.txt")
            .expect("read A for B's path");
    ...
    fn test_destroy_clean() {
        ...
        assert!(
            backend.exists(&id).expect("exists before destroy"),
            "must exist before destroy"
        );
        assert!(
            store.get(&id).expect("store get before destroy").is_some(),
            "must be in store before destroy"
        );
        ...
        assert!(
            ...
            "backend must not have overlay"
        );
        assert!(
            store.get(&id).expect("store get after destroy").is_none(),
            "store must have no record"
        );

        let list = list_workspaces(&store).expect("list after destroy");
        assert!(
            !list.iter().any(|w| w.id == id),
            "list must not contain destroyed workspace"
        );
    }
    ...
    fn test_ttl_sweep() {
        ...
        assert!(
            !backend
                .exists(&ephemeral_id)
                .expect("exists ephemeral after sweep"),
            "ephemeral overlay must be gone"
        );
        assert!(
            store
                .get(&ephemeral_id)
                .expect("store get ephemeral")
                .is_none(),
            "ephemeral record must be gone"
        );

        // Persistent survives.
        assert!(
            backend
                .exists(&persistent_id)
                .expect("exists persistent after sweep"),
            "persistent overlay must survive"
        );
        assert!(
            store
                .get(&persistent_id)
                .expect("store get persistent")
                .is_some(),
            "persistent record must survive"
        );
    }
