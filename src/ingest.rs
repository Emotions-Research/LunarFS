...
pub fn walk_repo(store: &dyn Store, repo_path: &Path) -> io::Result<Hash> {
    ...
    while bfs_pos < bfs_queue.len() {
        if bfs_queue.len() > MAX_WALK_ENTRIES {
            return Err(io::Error::other("repo has too many directories (max 65536)"));
        }
    ...
    for dir in dirs.iter().rev() {
        ...
        for entry in rd {
            ...
            if meta.is_dir() {
                ...
                tree_entries.push(TreeEntry { mode: MODE_DIR, name, hash: *child_hash });
            } else if meta.is_file() {
                let data = std::fs::read(&path)?;
                let hash = store.put(&data)?;
                tree_entries.push(TreeEntry { mode: file_mode(&meta), name, hash });
            }
    ...
    dir_hashes.get(&repo_path).copied().ok_or_else(|| io::Error::other("root tree hash not computed"))
}
