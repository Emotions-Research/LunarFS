...
pub async fn push(
    ...
    // Fetch the server's current root before uploading blobs so we can use it as
    // expected_root for compare-and-swap. This is the closest approximation to
    // "last synced root" available without a local per-workspace state file: it
    // detects concurrent writers that push between when we start and when we land
    // our own put_ref. If the workspace has no ref yet (404) we pass None and the
    // server treats it as a first push (unconditional accept).
    ...
    remote.put_ref_cas(workspace, root, expected_root.as_ref()).await?;
    Ok(upload_count)
}
    ...
    while let Some((hash, is_tree)) = stack.pop() {
        ...
        let data = if local.has(&hash) {
            local
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!("has() true but get() returned None for {}", hash_to_hex(&hash))
                })?
        } else if is_tree {
...
mod tests {
    ...
    fn remote_with_flat_tree(workspace: &str, blobs: &[(&str, &[u8])]) -> (Arc<Remote>, Hash) {
        ...
        let entries: Vec<TreeEntry> = blobs
            ...
            .map(|(name, data)| {
                ...
                TreeEntry { mode: MODE_FILE, name: name.to_string(), hash }
            })
            .collect();
    ...
    fn poll_once_updated_on_first_head() {
        let (remote, root) =
            remote_with_flat_tree("s2", &[("a.txt", b"a"), ("b.txt", b"b")]);
        let daemon = SyncDaemon::new(remote);
    ...
    fn poll_once_unchanged_when_head_stable() {
        ...
        assert!(matches!(daemon.poll_once().unwrap(), SyncOutcome::Unchanged));
    }

    #[test]
    fn poll_once_updated_after_head_advances() {
        let (remote, _root1) =
            remote_with_flat_tree("s4", &[("a.txt", b"a"), ("b.txt", b"b")]);
        ...
        let tree2 = serialize_tree(&[
            TreeEntry { mode: MODE_FILE, name: "c.txt".into(), hash: h_c },
            TreeEntry { mode: MODE_FILE, name: "d.txt".into(), hash: h_d },
        ]);
