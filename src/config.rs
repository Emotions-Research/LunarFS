...
pub fn config_path_with_env(env: &HashMap<String, String>) -> Result<PathBuf> {
    if let Some(dir) = env.get("LUNAR_CONFIG_HOME").filter(|s| !s.is_empty()) {
        ...
        assert!(!path.as_os_str().is_empty(), "resolved config path must not be empty");
        return Ok(path);
    }
    let home = home_dir_from_env(env)?;
    assert!(!home.as_os_str().is_empty(), "home directory path must not be empty");
...
pub fn load_config_from_path(path: &Path) -> Result<Config> {
    assert!(!path.as_os_str().is_empty(), "config path must not be empty");
