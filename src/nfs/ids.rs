...
impl IdTable {
        ...
        for (file_path, _hash) in index.entries() {
            ...
            for (i, byte) in file_path.as_bytes().iter().enumerate() {
                if *byte == b'/' {
                    ...
                    node_set
                        .entry(dir_prefix.to_owned())
                        .or_insert(NodeKind::Dir);
                    assert!(
    ...
    pub fn intern(&self, path: &str, kind: NodeKind) -> u64 {
        ...
        assert!(
            id > 0,
            "dynamic fileid must be positive and strictly increasing"
        );
        lock.next_id = id + 1;
    ...
    pub fn rename_path(&self, old: &str, new_path: &str, kind: NodeKind) {
        ...
        assert!(
            new_path.len() <= 4096,
            "new path must not exceed 4096 bytes"
        );
        let mut lock = self.dyn_maps.lock().expect("dyn_maps lock poisoned");
    ...
    pub fn rename_subtree(&self, old_prefix: &str, new_prefix: &str) {
        assert!(
            old_prefix.len() <= 4096,
            "old_prefix must not exceed 4096 bytes"
        );
        assert!(
            new_prefix.len() <= 4096,
            "new_prefix must not exceed 4096 bytes"
        );
...
fn intern_in_lock(base: &HashMap<String, u64>, lock: &mut DynamicMaps, path: &str, kind: NodeKind) {
    assert!(path.len() <= 4096, "path must not exceed 4096 bytes");
    lock.tombstones.remove(path);
...
mod tests {
    ...
    fn round_trip_all_paths() {
        ...
        for id in 1..=(table.len() as u64) {
            let (path, _kind) = table
                .path_of(id)
                .unwrap_or_else(|| panic!("id {} must exist", id));
            let back = table
                .id_of(&path)
    ...
    fn forget_hides_path() {
        ...
        let table = IdTable::build(&index);
        assert!(
            table.id_of("README.md").is_some(),
            "must be visible before forget"
        );
        table.forget("README.md");
        assert!(
            table.id_of("README.md").is_none(),
            "must be hidden after forget"
        );
        assert!(
            table.path_of(2).is_none(),
            "path_of must return None for tombstoned path"
        );
    }
    ...
    fn intern_clears_tombstone() {
        ...
        assert!(
            table.id_of("README.md").is_some(),
            "must be visible after intern clears tombstone"
        );
    }
    ...
    fn rename_subtree_rekeyes_nested_paths() {
        ...
        assert!(
            table.id_of("src/main.rs").is_none(),
            "src/main.rs must be tombstoned"
        );
        assert!(
            table.id_of("src/sub").is_none(),
            "src/sub must be tombstoned"
        );
        assert!(
            table.id_of("src/sub/lib.rs").is_none(),
            "src/sub/lib.rs must be tombstoned"
        );
        assert!(
            table.id_of("src/new.rs").is_none(),
            "src/new.rs must be tombstoned"
        );

        // New paths must be visible
        assert!(table.id_of("lib").is_some(), "lib must be interned");
        assert!(
            table.id_of("lib/main.rs").is_some(),
            "lib/main.rs must be interned"
        );
        assert!(table.id_of("lib/sub").is_some(), "lib/sub must be interned");
        assert!(
            table.id_of("lib/sub/lib.rs").is_some(),
            "lib/sub/lib.rs must be interned"
        );
        assert!(
            table.id_of("lib/new.rs").is_some(),
            "lib/new.rs must be interned"
        );
    }
    ...
    fn dynamic_ids_never_collide_with_base_ids() {
        let index = make_fixture_index();
        let table = IdTable::build(&index);
        let base_ids: std::collections::HashSet<u64> = (1..=6).collect();
        ...
        assert!(
            !base_ids.contains(&dyn_id1),
            "dynamic id must not collide with base"
        );
        assert!(
            !base_ids.contains(&dyn_id2),
            "dynamic id must not collide with base"
        );
        assert!(
            !base_ids.contains(&dyn_id3),
            "dynamic id must not collide with base"
        );
