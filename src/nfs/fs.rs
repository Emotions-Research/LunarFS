...
impl CasNfs {
    ...
    fn fattr_for(&self, id: fileid3, path: &str, kind: NodeKind) -> fattr3 {
        let default_t = nfstime3 { seconds: self.mount_time, nseconds: 0 };
    ...
    fn make_overlay_view(&self) -> OverlayView<'_> {
        let entries = self.overlay.entries_for_agent(self.agent).unwrap_or_default();
        let sizes_snap = {
            let s = self.overlay_sizes.lock().expect("overlay_sizes poisoned");
            s.clone()
        };
    ...
    fn read_full_for_rmw(&self, path: &str) -> std::io::Result<Vec<u8>> {
        ...
        match resolution {
            ...
            Resolution::Base => {
                let hash = self.index.lookup(path).expect("Base resolution with lookup hit");
                Ok(self.store.get(&hash)?.unwrap_or_default())
            }
        }
    }
    ...
    fn apply_sattr_overrides(&self, path: &str, attr: &sattr3) {
        ...
        let entry = overrides
            .entry(path.to_owned())
            .or_insert(AttrOverride { mode: None, mtime: None });
    ...
    fn has_children(&self, dir_path: &str) -> bool {
        assert!(dir_path.len() <= 4096, "dir_path must not exceed 4096 bytes");
    ...
    fn rename_file(&self, src: &str, dst: &str) -> Result<(), nfsstat3> {
        ...
        let bytes =
            self.read_full_for_rmw(src).map_err(|_| nfsstat3::NFS3ERR_IO)?;
    ...
    fn rename_dir(&self, src_prefix: &str, dst_prefix: &str) -> Result<(), nfsstat3> {
        assert!(src_prefix.len() <= 4096, "src_prefix must not exceed 4096 bytes");
        assert!(dst_prefix.len() <= 4096, "dst_prefix must not exceed 4096 bytes");
        ...
        for e in self.overlay.entries_for_agent(self.agent).unwrap_or_default() {
        ...
        for src_path in &file_paths {
            ...
            let bytes = self.read_full_for_rmw(src_path).map_err(|_| nfsstat3::NFS3ERR_IO)?;
...
impl NFSFileSystem for CasNfs {
    ...
    async fn lookup(&self, dirid: fileid3, filename: &filename3) -> Result<fileid3, nfsstat3> {
        let (dir_path, dir_kind) =
            self.ids.path_of(dirid).ok_or(nfsstat3::NFS3ERR_NOENT)?;
        if dir_kind != NodeKind::Dir {
            return Err(nfsstat3::NFS3ERR_NOTDIR);
        }
        let name = std::str::from_utf8(filename.as_ref())
            .map_err(|_| nfsstat3::NFS3ERR_INVAL)?;
        assert!(!name.is_empty(), "NFS lookup must not request empty filename");
    ...
    async fn readdir(
        ...
        for de in &raw_entries {
            ...
            let child_id = self.ids.id_of(&child_path)
        ...
        for (child_path, child_kind) in self.ids.dynamic_dir_children_of(&dir_path) {
            let child_name = child_path.rsplit('/').next().unwrap_or(&child_path).to_owned();
        ...
        assert!(children.len() <= 65_536, "child count must not exceed 65536");
    ...
    async fn write(
        ...
        let mut buf = self.read_full_for_rmw(&path).map_err(|_| nfsstat3::NFS3ERR_IO)?;
    ...
    async fn create(
        ...
        let child_path = join_path(&dir_path, name);
        assert!(child_path.len() <= 4096, "child path must not exceed 4096 bytes");
    ...
    async fn create_exclusive(
        ...
        let name = std::str::from_utf8(filename.as_ref())
            .map_err(|_| nfsstat3::NFS3ERR_INVAL)?;
        assert!(!name.is_empty(), "NFS create_exclusive must not have empty filename");
        let child_path = join_path(&dir_path, name);
        assert!(child_path.len() <= 4096, "child path must not exceed 4096 bytes");
    ...
    async fn mkdir(
        ...
        let name = std::str::from_utf8(dirname.as_ref())
            .map_err(|_| nfsstat3::NFS3ERR_INVAL)?;
    ...
    async fn rename(
        ...
        let (from_dir, from_dir_kind) =
            self.ids.path_of(from_dirid).ok_or(nfsstat3::NFS3ERR_NOENT)?;
        if from_dir_kind != NodeKind::Dir {
            return Err(nfsstat3::NFS3ERR_NOTDIR);
        }
        let (to_dir, to_dir_kind) =
            self.ids.path_of(to_dirid).ok_or(nfsstat3::NFS3ERR_NOENT)?;
        if to_dir_kind != NodeKind::Dir {
            return Err(nfsstat3::NFS3ERR_NOTDIR);
        }
        let from_name = std::str::from_utf8(from_filename.as_ref())
            .map_err(|_| nfsstat3::NFS3ERR_INVAL)?;
        let to_name = std::str::from_utf8(to_filename.as_ref())
            .map_err(|_| nfsstat3::NFS3ERR_INVAL)?;
        ...
        assert!(src_path.len() <= 4096, "src path must not exceed 4096 bytes");
        assert!(dst_path.len() <= 4096, "dst path must not exceed 4096 bytes");
...
mod tests {
    ...
    fn make_fixture() -> (Index, CountingStore, Arc<AtomicU32>) {
        ...
        let counting = CountingStore { inner: mem2, count: Arc::clone(&counter) };
    ...
    async fn getattr_readme_returns_nf3reg_correct_size() {
        ...
        assert!(matches!(attr.ftype, ftype3::NF3REG), "README.md must be NF3REG");
    ...
    async fn lookup_readme_from_root() {
        ...
        let found = fs.lookup(1, &b"README.md"[..].into()).await.expect("lookup must succeed");
    ...
    async fn lookup_on_file_returns_notdir() {
        ...
        let err = fs.lookup(file_id, &b"anything"[..].into()).await.unwrap_err();
    ...
    async fn lazy_hydration_counter_proof() {
        ...
        assert_eq!(counter.load(Ordering::Relaxed), 0, "no hydration after getattr");

        fs.readdir(1, 0, 100).await.unwrap();
        fs.readdir(src_id, 0, 100).await.unwrap();
        assert_eq!(counter.load(Ordering::Relaxed), 0, "no hydration after readdir");

        let (bytes, _eof) = fs.read(readme_id, 0, 1024).await.unwrap();
        assert_eq!(bytes, b"hello world");
        assert_eq!(counter.load(Ordering::Relaxed), 1, "exactly one hydration after read");
    }
    ...
    async fn readdir_root_returns_both_children() {
        ...
        assert!(!names.contains(&".".to_owned()), "dot entries must be excluded");
    }
    ...
    async fn write_existing_file_roundtrip() {
        ...
        assert_eq!(fattr.size, new_data.len() as u64, "returned fattr must show new size");
        ...
        let attr = fs.getattr(id).await.expect("getattr must succeed");
        assert_eq!(attr.size, new_data.len() as u64, "getattr must reflect new size");
        assert_ne!(bytes.as_slice(), b"hello world", "must not return base content");
    }
    ...
    async fn write_at_offset_extends_file() {
        ...
        fs.write(id, 11, extra).await.expect("offset write must succeed");
    ...
    async fn create_new_file_visible_in_lookup_readdir_read() {
        ...
        assert!(matches!(fattr.ftype, ftype3::NF3REG), "created file must be NF3REG");

        // lookup returns same id
        let looked_up = fs.lookup(1, &b"new.txt"[..].into()).await.unwrap();
        assert_eq!(looked_up, fid, "lookup must return the same fileid as create");
        ...
        let names: Vec<String> = result
            ...
            .map(|e| String::from_utf8_lossy(&e.name.0).to_string())
            .collect();
        assert!(names.contains(&"new.txt".to_owned()), "new.txt must appear in readdir");
    }
    ...
    async fn mkdir_then_getattr_and_parent_readdir() {
        ...
        assert!(matches!(dir_attr.ftype, ftype3::NF3DIR), "mkdir result must be NF3DIR");
        ...
        let names: Vec<String> = result
            ...
            .map(|e| String::from_utf8_lossy(&e.name.0).to_string())
            .collect();
        assert!(names.contains(&"newdir".to_owned()), "newdir must appear in parent readdir");

        // remove empty dir must succeed
        fs.remove(1, &b"newdir"[..].into()).await.expect("remove empty dir must succeed");
        let err = fs.getattr(dir_fid).await.unwrap_err();
        assert!(matches!(err, nfsstat3::NFS3ERR_NOENT), "getattr after remove must return NOENT");
    }
    ...
    async fn rename_file_old_gone_new_readable() {
        ...
        assert!(matches!(err, nfsstat3::NFS3ERR_NOENT), "old name must be gone");

        let new_id = fs.lookup(1, &b"NOTES.md"[..].into()).await.expect("new name must exist");
        let (bytes, _) = fs.read(new_id, 0, 1024).await.unwrap();
        assert_eq!(bytes.as_slice(), b"hello world", "renamed file must read original bytes");

        // old fileid must also be gone
        let err2 = fs.getattr(readme_id).await.unwrap_err();
        assert!(matches!(err2, nfsstat3::NFS3ERR_NOENT), "old fileid must return NOENT");
    }
    ...
    async fn rename_dir_subtree_rekeyes_contents() {
        ...
        // New names present
        let lib_id = fs.lookup(1, &b"lib"[..].into()).await.expect("lib must exist");
        let lib_attr = fs.getattr(lib_id).await.unwrap();
        assert!(matches!(lib_attr.ftype, ftype3::NF3DIR));

        let main_id = fs.lookup(lib_id, &b"main.rs"[..].into()).await.expect("lib/main.rs must exist");
    ...
    async fn remove_file_noent_and_readdir_excludes() {
        let (fs, _) = make_nfs_writable();
        let readme_id = fs.ids.id_of("README.md").unwrap();
        fs.remove(1, &b"README.md"[..].into()).await.expect("remove must succeed");
        ...
        let names: Vec<String> = result
            ...
            .map(|e| String::from_utf8_lossy(&e.name.0).to_string())
            .collect();
        assert!(!names.contains(&"README.md".to_owned()), "deleted file must not appear in readdir");
    }
    ...
    async fn cross_agent_isolation() {
        ...
        let fs_a = CasNfs::with_overlay(index_a, Box::new(store_a), Arc::clone(&overlay), agent_a, 1);

        let (index_b, store_b, _) = make_fixture();
        let agent_b = overlay.fork(1).expect("fork B");
        let fs_b = CasNfs::with_overlay(index_b, Box::new(store_b), Arc::clone(&overlay), agent_b, 1);

        fs_a.remove(1, &b"README.md"[..].into()).await.expect("agent A remove must succeed");

        // B should still see README.md
        let id_b = fs_b.ids.id_of("README.md").unwrap();
        let (bytes, _) = fs_b.read(id_b, 0, 100).await.expect("agent B must still read base file");
        assert_eq!(bytes.as_slice(), b"hello world", "agent B must see base bytes");
    }
    ...
    async fn setattr_size_shrink_and_grow() {
        ...
        let fattr = fs.setattr(id, attr).await.expect("setattr shrink must succeed");
        ...
        let fattr2 = fs.setattr(id, attr2).await.expect("setattr grow must succeed");
    ...
    async fn setattr_mode_reflected_in_fattr() {
        ...
        let fattr = fs.setattr(id, attr).await.expect("setattr mode must succeed");
        assert_eq!(fattr.mode & 0o7777, 0o600, "mode must be reflected in returned fattr");
        let attr2 = fs.getattr(id).await.unwrap();
        assert_eq!(attr2.mode & 0o7777, 0o600, "mode must be reflected in subsequent getattr");
    }
    ...
    async fn getattr_readdir_with_overlay_writes_zero_base_hydrations() {
        ...
        let (fid, _) = fs.create(1, &b"new.txt"[..].into(), sattr3::default()).await.unwrap();
        ...
        assert_eq!(counter.load(Ordering::Relaxed), 0, "no hydration after getattr with overlay");

        // readdir on root: must not hydrate any blob
        fs.readdir(1, 0, 100).await.unwrap();
        assert_eq!(counter.load(Ordering::Relaxed), 0, "no hydration after readdir with overlay");

        // Single read on the overlay-written file: exactly one hydration
        let (bytes, _) = fs.read(fid, 0, 100).await.unwrap();
        assert_eq!(bytes.as_slice(), b"overlay content");
        assert_eq!(counter.load(Ordering::Relaxed), 1, "exactly one hydration for overlay read");
    }
    ...
    async fn mkdir_create_file_rename_dir() {
        ...
        let bx_id = fs.lookup(b_id, &b"x"[..].into()).await.expect("b/x must exist");
    ...
    async fn write_to_base_file_succeeds() {
        let (fs, _) = make_nfs();
        let id = fs.ids.id_of("README.md").unwrap();
        let fattr = fs.write(id, 0, b"new data").await.expect("write must succeed");
    ...
    async fn mkdir_at_root_succeeds() {
        let (fs, _) = make_nfs();
        let (fid, fattr) = fs.mkdir(1, &b"newdir"[..].into()).await.expect("mkdir must succeed");
        assert!(fid > 0);
