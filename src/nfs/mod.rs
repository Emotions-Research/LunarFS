...
pub fn mount_nfs(repo: &Path, mountpoint: &Path) -> Result<()> {
    anyhow::ensure!(repo.is_dir(), "repo must be an existing directory: {}", repo.display());
    anyhow::ensure!(
        mountpoint.is_dir(),
    ...
    rt.block_on(async {
        ...
        eprintln!("nfs: mounted {} at {}", repo.display(), mountpoint.display());

        // RAII guard: unmounts on every exit path below, including panic unwinds.
        let _guard = MountGuard { mountpoint: mountpoint.to_path_buf() };
