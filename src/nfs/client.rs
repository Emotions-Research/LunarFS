...
fn run_unmount_command(os: ClientOs, mountpoint: &Path, force: bool) -> Result<()> {
    ...
    let status = std::process::Command::new(&program)
        ...
        .map_err(|e| anyhow::anyhow!("failed to launch {}: {}", program, e))?;
    if !status.success() {
        anyhow::bail!("{} exited with {:?}", program, status.code());
    }
    Ok(())
}
...
mod tests {
    ...
    fn macos_argv_uses_mount_nfs_with_required_opts() {
        ...
        let opts = args
            .iter()
            .find(|a| a.contains("vers=3"))
            .expect("opts must contain vers=3");
        ...
        assert!(
            opts.contains("mountport=2049"),
            "opts must specify mountport"
        );
    ...
    fn macos_argv_source_is_root_export() {
        ...
        let (_program, args) =
            build_nfs_mount_argv(ClientOs::MacOs, "127.0.0.1", 5000, &mountpoint);
        // Source "127.0.0.1:/" must appear before the mountpoint.
        let source_pos = args
            .iter()
            .position(|a| a == "127.0.0.1:/")
            .expect("source must be present");
        let mnt_pos = args
            .iter()
            .position(|a| a == "/mnt/foo")
            .expect("mountpoint must be present");
    ...
    fn linux_argv_uses_mount_t_nfs() {
        ...
        assert!(
            opts.contains("mountport=5555"),
            "opts must specify mountport"
        );
        assert!(
            opts.contains("nolock"),
            "Linux opts must use nolock (not nolocks)"
        );
    }
