...
pub fn merge_snapshots(
    ...
    assert!(!suffix.is_empty(), "conflict suffix must not be empty for merge");
    ...
    for p in base.keys() { all_paths.insert(p); }
    for p in winner.keys() { all_paths.insert(p); }
    for p in local.keys() { all_paths.insert(p); }
    ...
    for path in all_paths {
        ...
        outcomes.push(PathOutcome { path: path.clone(), action });
    }

    MergeResult { merged, outcomes, conflicts }
}
...
fn apply_conflict(
    ...
    assert!(!path.is_empty(), "path must not be empty in conflict resolution");
...
mod tests {
    ...
    fn snap(entries: &[(&str, &str)]) -> Snapshot {
        entries.iter().map(|(p, c)| (p.to_string(), FileState::Present(c.to_string()))).collect()
    }
    ...
    fn conflict_copy_path_with_extension() {
        assert_eq!(conflict_copy_path("foo.txt", "x"), "foo (conflicted copy x).txt");
    }

    #[test]
    fn conflict_copy_path_no_extension() {
        assert_eq!(conflict_copy_path("Makefile", "y"), "Makefile (conflicted copy y)");
    }

    #[test]
    fn conflict_copy_path_hidden_file() {
        assert_eq!(conflict_copy_path(".hidden", "z"), ".hidden (conflicted copy z)");
    }

    #[test]
    fn conflict_copy_path_with_dir() {
        assert_eq!(conflict_copy_path("dir/foo.txt", "s"), "dir/foo (conflicted copy s).txt");
    }
    ...
    fn disjoint_changes_auto_merge() {
        ...
        assert!(result.conflicts.is_empty(), "disjoint changes must not conflict");
    ...
    fn same_content_both_sides_no_conflict() {
        ...
        let result = merge_snapshots(&base, &winner, &local, "s");
        assert!(result.conflicts.is_empty(), "identical edits must not conflict");
    ...
    fn winner_deletes_local_edits_gives_conflict_copy() {
        ...
        assert!(!result.merged.contains_key("bar.txt"), "original must be absent");
        let copy = conflict_copy_path("bar.txt", "s1");
        assert_eq!(result.merged[&copy], FileState::Present("local-edit".into()));
    }
    ...
    fn winner_edits_local_deletes_edit_survives() {
        ...
        assert!(result.conflicts.is_empty(), "no conflict copy when edit wins over delete");
        assert_eq!(result.merged["bar.txt"], FileState::Present("winner-edit".into()));
    }
    ...
    fn new_file_added_by_winner_only() {
        ...
        let result = merge_snapshots(&base, &winner, &local, "s");
        assert!(result.conflicts.is_empty());
        assert_eq!(result.merged["new.txt"], FileState::Present("content".into()));
    }
    ...
    fn deleted_state_not_inserted_into_merged() {
        ...
        let result = merge_snapshots(&base, &winner, &local, "s");
        assert!(result.conflicts.is_empty());
        assert!(!result.merged.contains_key("f.txt"), "both deleted: path must be absent");
    }
}
