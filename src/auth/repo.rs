...
pub fn add_membership(conn: &Connection, user_id: i64, org_id: i64, role: Role) -> Result<()> {
...
pub fn add_membership_checked(
    ...
    #[cfg(feature = "hosted")]
    {
        ...
        check_seat_available(plan, count).map_err(|e| anyhow::anyhow!(e.to_string()))?;
    }
...
mod tests {
    ...
    fn fk_enforcement_rejects_unknown_ids() {
        ...
        assert!(
            err.is_err(),
            "FK constraint must reject non-existent user_id"
        );
        ...
        assert!(
            err2.is_err(),
            "FK constraint must reject non-existent org_id"
        );
    }
    ...
    fn workspace_owner_user_and_org_and_unknown() {
        ...
        let wid_user = create_workspace(&conn, "my-workspace", OwnerKind::User, uid, 10).unwrap();
        let wid_org = create_workspace(&conn, "org-workspace", OwnerKind::Org, oid, 20).unwrap();
    ...
    fn workspace_by_name_returns_lowest_id_on_duplicate_name() {
        ...
        assert_eq!(
            result,
            Some(id1),
            "must return the lowest id among duplicates"
        );
    }
