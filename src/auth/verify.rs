...
mod tests {
    ...
    fn no_clerk_returns_unsupported() {
        ...
        let err = v
            .verify("any-bearer")
            .expect_err("NoClerk must reject any credential");
        assert!(
            matches!(err, VerifyError::Unsupported),
            "expected Unsupported, got {err:?}"
        );
        let err2 = v
            .verify("Bearer token")
            .expect_err("NoClerk must reject Bearer-prefixed");
    ...
    fn auth_mode_from_env_selects_self_host_when_vars_absent() {
        ...
        let err = verifier
            .verify("any")
            .expect_err("self-host mode must reject JWT");
