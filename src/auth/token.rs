...
pub fn mint(
    ...
    assert!(created_at >= 0, "clock must return a non-negative unix timestamp");
...
pub fn validate(
    ...
    let kind = str_to_kind(&kind_str).ok_or_else(|| {
        AuthError::Internal(format!("unknown principal_kind in db: {kind_str}"))
    })?;

    Ok(Principal { kind, id: principal_id })
}
...
pub fn revoke(
    ...
    assert!(revoked_at >= 0, "clock must return a non-negative unix timestamp");
...
mod tests {
    ...
    fn roundtrip_mint_validate() {
        ...
        let principal = Principal { kind: PrincipalKind::User, id: "42".to_string() };

        let minted = mint(&conn, &principal, None, None, &clock).unwrap();
        assert!(minted.plaintext.starts_with("ddb_"), "plaintext must carry the ddb_ prefix");
    ...
    fn wrong_tokens_rejected() {
        ...
        assert!(matches!(err1, Err(AuthError::NotFound)), "wrong token must be NotFound");

        let err2 = validate(&conn, "completelymalformedstring", &clock);
        assert!(matches!(err2, Err(AuthError::NotFound)), "malformed token must be NotFound");
    }
    ...
    fn revoked_token_rejected() {
        let (_dir, conn) = setup();
        let clock = FakeClock { now: 1_000_000 };
        let principal = Principal { kind: PrincipalKind::Org, id: "7".to_string() };

        let minted = mint(&conn, &principal, None, None, &clock).unwrap();
        revoke(&conn, minted.token_id, &clock).unwrap();

        let err = validate(&conn, &minted.plaintext, &clock);
        assert!(matches!(err, Err(AuthError::Revoked)), "revoked token must return Revoked");
    }
    ...
    fn expired_token_rejected() {
        let (_dir, conn) = setup();
        let clock = FakeClock { now: 1_000_000 };
        let principal = Principal { kind: PrincipalKind::User, id: "99".to_string() };
        ...
        assert!(matches!(err, Err(AuthError::Expired)), "expired token must return Expired");
    }
    ...
    fn plaintext_not_persisted() {
        let (_dir, conn) = setup();
        let clock = FakeClock { now: 1_000_000 };
        let principal = Principal { kind: PrincipalKind::User, id: "1".to_string() };
