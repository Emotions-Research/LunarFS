...
pub fn grant(
    ...
    assert!(path_prefix.len() <= 4096, "path_prefix must not exceed 4096 bytes");
...
pub fn revoke(conn: &Connection, grant_id: i64, revoked_at: i64) -> Result<()> {
    ...
    assert!(revoked_at >= 0, "revoked_at must be a non-negative unix timestamp");
    ...
    if rows == 0 {
        return Err(anyhow::anyhow!("grant {} not found or already revoked", grant_id));
    }
    Ok(())
}
...
pub fn authorize(
    ...
    assert!(grants.len() <= 1_000_000, "grant count exceeds safe cap of 1M");
...
mod tests {
    ...
    fn read_grant_blocks_write() {
        ...
        let gid =
            grant(&conn, PrincipalKind::User, &uid.to_string(), ws, "/data/", Permission::Read, 1)
                .unwrap();
        assert!(gid > 0);

        let principal = Principal { kind: PrincipalKind::User, id: uid.to_string() };
    ...
    fn write_grant_allows_read_and_write() {
        ...
        let ws = repo::create_workspace(&conn, "ws", crate::auth::OwnerKind::User, 1, 1).unwrap();
        let uid = repo::create_user(&conn, None, 1).unwrap();
        grant(&conn, PrincipalKind::User, &uid.to_string(), ws, "/repo/", Permission::Write, 1)
            .unwrap();
        ...
        assert_eq!(
            authorize(&conn, &principal, ws, "/repo/src/main.rs", Permission::Write).unwrap(),
            Decision::Allow,
    ...
    fn cross_workspace_denied() {
        ...
        grant(&conn, PrincipalKind::User, &uid.to_string(), ws_a, "/shared/", Permission::Read, 1)
            .unwrap();
    ...
    fn path_prefix_boundary() {
        ...
        let uid = repo::create_user(&conn, None, 1).unwrap();
        let ws =
            repo::create_workspace(&conn, "ws", crate::auth::OwnerKind::User, uid, 1).unwrap();

        // Grant on /a only.
        grant(&conn, PrincipalKind::User, &uid.to_string(), ws, "/a", Permission::Read, 1)
            .unwrap();
    ...
    fn org_member_allowed_via_org_grant() {
        ...
        let ws =
            repo::create_workspace(&conn, "ws", crate::auth::OwnerKind::Org, oid, 1).unwrap();
        ...
        let member = Principal { kind: PrincipalKind::User, id: uid.to_string() };
        assert_eq!(
        ...
        let outsider = Principal { kind: PrincipalKind::User, id: uid2.to_string() };
        assert_eq!(
    ...
    fn revoked_grant_denied() {
        ...
        let gid =
            grant(&conn, PrincipalKind::User, &uid.to_string(), ws, "/secret/", Permission::Read, 1)
                .unwrap();
