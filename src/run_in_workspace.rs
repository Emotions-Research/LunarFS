    ...
    F: FnOnce(&B::Mount) -> Disposition<T>,
{
    ...
    let mut guard = CleanupGuard { backend, id, deactivated: false };
    ...
    let run_result =
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| f(&mount_token)));
...
mod tests {
    use super::{
        Disposition, Outcome, PromotedRef, RunError, WorkspaceBackend, WorkspaceId,
        run_in_workspace,
    };
    ...
    impl WorkspaceBackend for InMemBackend {
        ...
        fn promote(&self, id: &WorkspaceId) -> Result<PromotedRef, Self::Error> {
            ...
            assert!(promoted.len() <= 10_000, "promoted count exceeds sanity cap");
        ...
        fn destroy(&self, id: &WorkspaceId) -> Result<(), Self::Error> {
            ...
            self.overlays.lock().expect("overlays lock poisoned").remove(&id.0);
            Ok(())
        }
    }
    ...
    fn keep_path_promotes_and_cleans_overlay() {
        ...
        assert_eq!(backend.promoted_count(), 1, "exactly one promoted ref after Keep");
    }
    ...
    fn discard_path_destroys_overlay_only() {
        let backend = InMemBackend::new();
        let result =
            run_in_workspace::<_, _, ()>(&backend, noop_base(), |_| Disposition::Discard);
        assert!(
            matches!(result, Ok(Outcome::Discarded)),
            "discard must return Ok(Discarded)"
        );
        assert_eq!(backend.live_overlays(), 0, "no orphan overlays after Discard");
        assert_eq!(backend.promoted_count(), 0, "no promoted refs after Discard");
    }
    ...
    fn panic_path_destroys_overlay_and_returns_error() {
        ...
        assert_eq!(backend.live_overlays(), 0, "no orphan overlays after closure panic");
        assert_eq!(backend.promoted_count(), 0, "no promoted refs after closure panic");
    }
    ...
    fn mount_failure_destroys_forked_overlay() {
        ...
        assert_eq!(backend.live_overlays(), 0, "no orphan overlays after mount failure");
        assert_eq!(backend.promoted_count(), 0, "no promoted refs after mount failure");
    }
    ...
    fn promote_failure_destroys_overlay() {
        ...
        assert_eq!(backend.live_overlays(), 0, "no orphan overlays after promote failure");
        assert_eq!(backend.promoted_count(), 0, "no promoted refs after promote failure");
    }
    ...
    fn no_orphan_invariant_across_multiple_runs() {
        ...
        assert_eq!(backend.promoted_count(), 5, "five promoted refs after five keep runs");
    }
    ...
    fn fork_generates_deterministic_ids() {
        ...
        assert_eq!(id1, WorkspaceId("ws-1".to_string()), "first id must be ws-1");
        assert_eq!(id2, WorkspaceId("ws-2".to_string()), "second id must be ws-2");
        assert_eq!(backend.live_overlays(), 0, "no orphans after direct fork+destroy test");
    }
    ...
    fn fork_failure_returns_error_and_no_orphan() {
        ...
        assert_eq!(backend.live_overlays(), 0, "no orphan overlay after fork failure");
    }
}
