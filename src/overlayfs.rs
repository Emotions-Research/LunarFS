...
pub fn overlay_options(lower: &Path, upper: &Path, work: &Path) -> String {
    ...
    assert!(s.starts_with("lowerdir="), "overlay_options must start with lowerdir=");
    assert!(s.contains(",upperdir="), "overlay_options must contain ,upperdir=");
    s
}
...
fn ensure_upper_work(upper: &Path, work: &Path) -> Result<()> {
    ...
    assert!(upper.is_dir(), "upper dir must exist after ensure_upper_work");
...
mod tests {
    ...
    fn overlay_options_absolute_paths() {
        ...
        assert_eq!(
            opts,
            "lowerdir=/srv/base,upperdir=/agent/1/upper,workdir=/agent/1/work",
    ...
    fn mount_overlay_privilege_gated() {
        ...
        mount_overlay(&lower, &upper, &work, &target)
            .expect("mount_overlay must succeed as root");
