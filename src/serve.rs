...
impl From<crate::billing::entitlement::EntitlementError> for ApiError {
    fn from(e: crate::billing::entitlement::EntitlementError) -> Self {
        ...
        match e {
            EntitlementError::StorageHardCapExceeded { limit_gb, attempted_gb } => {
...
pub fn build_object_store(spec: &str) -> anyhow::Result<Arc<dyn ObjectStore>> {
    ...
    if let Some(bucket) = spec.strip_prefix("s3://") {
        ...
        let store = builder.build().map_err(|e| anyhow::anyhow!(e.to_string()))?;
        return Ok(Arc::new(store));
    }

    bail!("unknown store spec {:?}: use local:<path> or s3://<bucket>", spec)
}
...
fn authorize_op(
    ...
    let conn =
        state.db.lock().map_err(|_| ApiError::Backend("db mutex poisoned".to_string()))?;
    ...
    let principal = match token_result {
        ...
        Err(_) => {
            ...
            match state.verifier.verify(bearer) {
                Ok(p) => AclPrincipal { kind: PrincipalKind::User, id: p.user_id },
...
async fn put_blob(
    ...
    let result = state
        .store
        .put_opts(&path, payload, PutOptions { mode: PutMode::Create, ..Default::default() })
        .await;
    match result {
...
async fn get_blob(
    ...
    match state.store.get(&path).await {
        Ok(result) => {
            let bytes =
                result.bytes().await.map_err(|e| ApiError::Backend(e.to_string()))?;
            let mut hdrs = HeaderMap::new();
            hdrs.insert(
                header::CONTENT_TYPE,
                "application/octet-stream".parse().expect("static header is valid"),
            );
...
async fn presign_blob(
    ...
    let ws_name = {
        let ws_q = WorkspaceQuery { workspace: q.workspace.clone() };
    ...
    let p = state
        ...
        .map_err(|e| ApiError::Backend(e.to_string()))?;
    Ok(Json(json!({"url": p.url, "method": p.method, "expires_at": p.expires_at})))
}
...
enum PutRefDecision {
    ...
    Conflict { short_hash: String, current_root: String },
}
...
fn decide_put_ref(
    ...
    if cur == exp {
        ...
        PutRefDecision::Conflict { short_hash, current_root: cur.to_string() }
    }
}
...
async fn put_ref(
    ...
    assert!(!workspace.is_empty(), "workspace path param must not be empty");
            ...
            match decide_put_ref(current_root_opt.as_deref(), Some(expected), &req.root) {
                PutRefDecision::Overwrite => {
                    ...
                    if current_root_opt.is_none() {
                        ...
                        let upd_version = UpdateVersion { e_tag: etag, version };
                        let result = state
                            .store
                            .put_opts(
                                &path,
                        ...
                        match result {
                            ...
                            | Err(object_store::Error::Precondition { .. }) => {
                                ...
                                let current =
                                    read_ref_root(&state.store, &path).await?;
                ...
                PutRefDecision::Conflict { short_hash, current_root } => {
...
async fn read_ref_root(
    store: &dyn ObjectStore,
    path: &ObjPath,
) -> Result<String, ApiError> {
    let result = store.get(path).await.map_err(|e| ApiError::Backend(e.to_string()))?;
    let bytes = result.bytes().await.map_err(|e| ApiError::Backend(e.to_string()))?;
...
async fn post_workspace_acl(
    ...
    let permission = parse_permission_str(&req.permission).ok_or_else(|| {
        ApiError::BadRequest(format!("invalid permission: {}", req.permission))
    })?;
    if req.principal_id.is_empty() {
        return Err(ApiError::BadRequest("principal_id must not be empty".to_string()));
    }
...
async fn get_workspace_acl(
    ...
    let grants = {
        ...
        acl::list_for_workspace(&conn, ws_id)
            .map_err(|e| ApiError::Backend(e.to_string()))?
    };
...
async fn post_workspace_fork(
    ...
    if req.new_workspace.is_empty() {
        return Err(ApiError::BadRequest("new_workspace must not be empty".to_string()));
    }
    ...
    assert!(!base_root.is_empty(), "resolved base root hash must not be empty");
    ...
    Ok((StatusCode::CREATED, Json(json!({"workspace": req.new_workspace, "root": base_root}))))
}
...
fn workspace_json(ws: &crate::workspace::Workspace) -> serde_json::Value {
    assert!(!ws.id.0.is_empty(), "workspace id must not be empty in workspace_json");
    json!({
        "id": ws.id.0,
        "label": ws.label,
...
pub fn build_router(state: AppState) -> Router {
    ...
    let authed = Router::new()
        ...
        .route("/v1/workspaces/:workspace/acl/:grant_id", delete(delete_workspace_acl))
    ...
    #[cfg(feature = "hosted")]
    let authed = authed
        .route("/v1/billing/usage", get(crate::billing::metering::get_billing_usage))
        .route("/v1/billing/checkout", post(crate::billing::provider::post_checkout))
        .route("/v1/billing/portal", post(crate::billing::provider::post_portal));

    let authed =
        authed.layer(middleware::from_fn_with_state(state.clone(), resolve_principal));
...
mod tests {
    ...
    fn no_expected_always_overwrites() {
        assert_eq!(decide_put_ref(Some(ROOT_A), None, ROOT_B), PutRefDecision::Overwrite);
        assert_eq!(decide_put_ref(None, None, ROOT_B), PutRefDecision::Overwrite);
    }

    #[test]
    fn first_push_with_expected_overwrites() {
        // No current ref: accept regardless of expected_root.
        assert_eq!(decide_put_ref(None, Some(ROOT_A), ROOT_B), PutRefDecision::Overwrite);
    }
