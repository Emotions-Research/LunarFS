...
impl NotifyChannel for InProcessNotifyChannel {
    fn subscribe(
        ...
        let h: HandlerFn = Arc::from(handler);
        {
            let mut inner = self
                .inner
                .lock()
                .expect("InProcessNotifyChannel subscribe lock");
    ...
    fn publish(&self, event: RefMoveEvent) {
        assert!(
            ...
            "publish: workspace_id must not be empty"
        );
        assert!(
            !event.snapshot_id.is_empty(),
            "publish: snapshot_id must not be empty"
        );
        ...
        let handlers: Vec<HandlerFn> = {
            let inner = self
                .inner
                .lock()
                .expect("InProcessNotifyChannel publish lock");
            if inner.closed {
                return;
            }
            inner
    ...
    fn close(&self) {
        let mut inner = self
            .inner
            .lock()
            .expect("InProcessNotifyChannel close lock");
...
impl ServerApi for MemServerApi {
    fn get_ref(&self, workspace_id: &str) -> Option<SnapshotId> {
        assert!(
            ...
            "get_ref: workspace_id must not be empty"
        );
        self.state
            .lock()
            .expect("MemServerApi get_ref lock")
            .refs
            .get(workspace_id)
            .cloned()
    }
    ...
    fn get_snapshot(&self, id: &str) -> Option<LiveSnapshot> {
        ...
        self.state
            .lock()
            .expect("MemServerApi get_snapshot lock")
            .snapshots
            .get(id)
            .cloned()
    }
    ...
    fn get_blob(&self, hash: &str) -> Option<Vec<u8>> {
        ...
        self.state
            .lock()
            .expect("MemServerApi get_blob lock")
            .blobs
            .get(hash)
            .cloned()
    }
    ...
    fn advance_ref(&self, workspace_id: &str, snapshot_id: &str) {
        assert!(
            !workspace_id.is_empty(),
            "advance_ref: workspace_id must not be empty"
        );
        assert!(
            !snapshot_id.is_empty(),
            "advance_ref: snapshot_id must not be empty"
        );
        {
            let mut st = self.state.lock().expect("MemServerApi advance_ref lock");
            st.refs
                .insert(workspace_id.to_string(), snapshot_id.to_string());
        }
...
impl ClientReconciler {
    ...
    pub fn reconcile(
        ...
        assert!(
            !workspace_id.is_empty(),
            "reconcile: workspace_id must not be empty"
        );
        assert!(
            !snapshot_id.is_empty(),
            "reconcile: snapshot_id must not be empty"
        );
        ...
        // Idempotent: already at this snapshot.
        {
            let st = self
                .state
                .lock()
                .expect("ClientReconciler reconcile lock (idempotent)");
        ...
        let (missing, cached): (Vec<BlobHash>, Vec<BlobHash>) = {
            let st = self
                .state
                .lock()
                .expect("ClientReconciler reconcile partition lock");
            let mut m = Vec::new();
            let mut c = Vec::new();
        ...
        // Commit: add newly fetched blobs to cache, swap mount.
        {
            let mut st = self
                .state
                .lock()
                .expect("ClientReconciler reconcile commit lock");
            for hash in &fetched {
    ...
    pub fn subscribe_live(
        ...
        assert!(
            !workspace_id.is_empty(),
            "subscribe_live: workspace_id must not be empty"
        );
        let ws = workspace_id.to_string();
        channel.subscribe(
            workspace_id,
    ...
    pub fn current_mount(&self) -> ClientMount {
        self.state
            .lock()
            .expect("ClientReconciler current_mount lock")
            .mount
            .clone()
    }
}
...
impl PollingFallback {
    pub fn new(
        ...
        assert!(
            !workspace_id.is_empty(),
            "PollingFallback: workspace_id must not be empty"
        );
        Self {
            reconciler,
            workspace_id,
    ...
    pub fn tick(&self) -> Option<ReconcileResult> {
        assert!(
            ...
            "tick: workspace_id must not be empty"
        );
        ...
        let changed = {
            let last = self
                .last_seen
                .lock()
                .expect("PollingFallback last_seen lock");
        ...
        match self.reconciler.reconcile(&self.workspace_id, &current) {
            Ok(result) => {
                *self
                    .last_seen
                    .lock()
                    .expect("PollingFallback last_seen update") = Some(current);
                Some(result)
            }
            Err(e) => {
...
impl NotifyChannel for SocketNotifyChannel {
    fn subscribe(
        ...
        assert!(
            !workspace_id.is_empty(),
            "subscribe: workspace_id must not be empty"
        );
        let sub_id = self.next_id.fetch_add(1, Ordering::Relaxed);
        let h: HandlerFn = Arc::from(handler);
        {
            let mut hs = self
                .handlers
                .lock()
                .expect("SocketNotifyChannel subscribe lock");
            hs.entry(workspace_id.to_string())
                .or_default()
                .push((sub_id, h));
        }
    ...
    fn publish(&self, event: RefMoveEvent) {
        ...
        match &self.mode {
            SocketMode::Server { connections, .. } => {
                let mut json = serde_json::to_string(&event).expect("RefMoveEvent must serialize");
                json.push('\n');
    ...
    fn close(&self) {
        ...
        self.handlers
            .lock()
            .expect("SocketNotifyChannel close lock")
            .clear();
    }
}
...
mod tests {
    ...
    impl CountingServerApi {
        ...
        fn get_blob_count(&self, hash: &str) -> usize {
            *self
                .counts
                .lock()
                .expect("CountingServerApi counts lock")
                .get(hash)
                .unwrap_or(&0)
        }
    }
    ...
    fn live_path_fetches_only_missing_blobs() {
...

        {
            ...
            assert!(
                r.skipped_blobs.contains(&"h1".to_string()),
                "h1 must be skipped in S2"
            );
        }

        // h1 must NOT have been refetched.
        assert_eq!(
            spy.get_blob_count("h1"),
            1,
            "h1 must NOT be refetched for S2"
        );
    ...
    fn polling_fallback_reaches_same_state_as_live() {
        ...
        assert_eq!(
            fetched,
            vec!["h1", "h3", "h4"],
            "poll must fetch all S2 blobs (cache was empty)"
        );

        // Second tick: same ref, no change, no reconcile.
        assert!(
            fallback.tick().is_none(),
            "second tick with same ref must be no-op"
        );
    ...
    fn reconcile_to_same_snapshot_is_noop() {
        ...
        rec.reconcile("ws1", "s1")
            .expect("first reconcile must succeed");
        assert_eq!(
            spy.get_blob_count("h1"),
            1,
            "h1 fetched once on first reconcile"
        );
        ...
        assert!(
            r.fetched_blobs.is_empty(),
            "second reconcile must fetch nothing"
        );
    ...
    fn socket_smoke() {
        ...
        assert!(
            ...
            "client must reconcile within 1s over TCP"
        );
        let mount = reconciler.current_mount();
